...
class SubdivisionGrid:
    ...
    @classmethod
    def create_tuplet(
        cls, n: int, in_space_of: int = 0, level: str = "quarter"
    ) -> "SubdivisionGrid":
        """
    ...
    @classmethod
    def create_swing(
        cls, swing_ratio: float, level: str = "eighth"
    ) -> "SubdivisionGrid":
        """
...
def generate_all_grids(
    ...
    grids["thirtysecond"] = SubdivisionGrid(
        "thirtysecond", 8, 0.125, SubdivisionType.STRAIGHT
    )
    grids["sixtyfourth"] = SubdivisionGrid(
        "sixtyfourth", 16, 0.0625, SubdivisionType.STRAIGHT
    )
    ...
    for n in range(3, max_tuplet + 1):
        ...
        if include_compound_tuplets:
            # Eighth-level tuplet
            grids[f"eighth_{n}tuplet"] = SubdivisionGrid.create_tuplet(
                n, level="eighth"
            )

            # Sixteenth-level (for very fast passages)
            if n <= 7:  # Don't go too crazy
                grids[f"sixteenth_{n}tuplet"] = SubdivisionGrid.create_tuplet(
                    n, level="sixteenth"
                )

    # Swing grids
    for ratio in swing_ratios:
        grids[f"eighth_swing_{ratio:.2f}"] = SubdivisionGrid.create_swing(
            ratio, "eighth"
        )
        grids[f"sixteenth_swing_{ratio:.2f}"] = SubdivisionGrid.create_swing(
            ratio, "sixteenth"
        )
...
def discover_tuplet_ratio(
    ...
    times = np.array(sorted(hit_times))
    beat_duration = 60.0 / bpm
    _tolerance = tolerance_ms / 1000.0  # Reserved for future tolerance-based matching
...
def detect_dynamic_time_signature(
    ...
    triplet_count = np.sum(
        (np.abs(beat_fractions - 1 / 3) < 0.1) | (np.abs(beat_fractions - 2 / 3) < 0.1)
    )
    triplet_ratio = (
...
def _detect_additive_grouping(
    ...
    iois = np.diff(times)
    # Note: beat_fractions reserved for future accent-based analysis
    _beat_fractions = iois / beat_duration

    # Note: eighth_duration reserved for compound meter analysis
    _eighth_duration = beat_duration / 2
...
def detect_polyrhythm(
    ...
    return PolyrhythmAnalysis(
        ...
        confidence=best_score,
        pattern_description=f"{best_pair[0]}:{best_pair[1]} polyrhythm"
        if is_poly
        else "no polyrhythm",
    )
...
def comprehensive_rhythm_analysis(
    ...
    primary_grid = SUBDIVISION_GRIDS.get(
        subdivision_analysis.best_grid, SUBDIVISION_GRIDS["sixteenth"]
    )
    ...
    beat_duration = 60.0 / bpm
    _section_duration = beat_duration * 16  # Reserved for section boundaries
...
def quantize_with_dynamic_grid(
    ...
    return QuantizationResult(
        ...
        swing_ratio=swing_ratio,
        tuplet_detected=optimal_grid.subdivision_type
        in [
            SubdivisionType.TRIPLET,
            SubdivisionType.TUPLET,
            SubdivisionType.QUINTUPLET,
            SubdivisionType.SEPTUPLET,
        ],
        ...
        is_polyrhythmic=polyrhythm.is_polyrhythmic,
        polyrhythm_grids=polyrhythm.active_grids
        if polyrhythm.is_polyrhythmic
        else None,
