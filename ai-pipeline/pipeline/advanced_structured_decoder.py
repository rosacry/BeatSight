...
from typing import Dict, List, Optional, Tuple, Sequence, Union, TYPE_CHECKING
...
if HAS_TORCH:
    ...
    class PositionalEncoding(nn.Module):
        ...
        def forward(self, x: torch.Tensor, beat_positions: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    class TransformerSequenceDecoder(nn.Module):
        ...
        def decode_greedy(
            ...
            logits = self.forward(emission_probs, confidences, time_deltas, beat_positions)
    ...
    class TransformerDecoderWrapper:
        ...
        def decode(
            ...
            for i, event in enumerate(events):
                ...
                beat_positions[0, i] = self.get_beat_position(event.get('time', 0), offset)
            ...
            # Decode
            states = self.model.decode_greedy(
...
class CRFDecoder:
    ...
    def decode(
        ...
        for t in range(1, n_events):
            ...
            trans_pots = self._get_transition_potentials(beat_idx, current_time - last_time)
...
class EnsembleDecoder:
    ...
    def __init__(
        ...
        if 'crf' in decoders:
            self.decoders['crf'] = CRFDecoder(n_states=len(DrumState), bpm=bpm, time_signature=time_signature)
    ...
    def decode(
        ...
        for t, event in enumerate(events):
            ...
            beat_idx = min(
                int(position_in_measure / beat_duration),
                self.time_signature[0] - 1
            )
...
def apply_advanced_structured_decoding(
    ...
    **kwargs,
    ...
    if decoder_type == "viterbi":
        ...
        decoder = CRFDecoder(n_states=len(DrumState), bpm=bpm, time_signature=time_signature)
    elif decoder_type == "transformer" and HAS_TORCH:
        decoder = TransformerDecoderWrapper(bpm=bpm, time_signature=time_signature, **kwargs)
