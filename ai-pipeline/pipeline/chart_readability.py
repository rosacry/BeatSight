...
class ReadabilityRules:
    ...
    max_nps_by_difficulty: Dict[str, float] = field(default_factory=lambda: {
        "easy": 4.0,
        "normal": 6.0,
        "hard": 10.0,
        "expert": 16.0,
    ...
    rest_requirements: Dict[int, float] = field(default_factory=lambda: {
...
class ChartReadabilityFilter:
    ...
    def __init__(
        ...
        self.max_nps = self.rules.max_nps_by_difficulty.get(
            self.difficulty, 
            self.rules.max_nps_by_difficulty["expert"]
        )
    ...
    def check_physical_constraints(
        ...
        for hit in sorted_hits:
            ...
            if limb != Limb.EITHER_HAND and limb in last_limb_time:
                ...
                min_ioi = (self.constraints.min_ioi_foot 
                          if limb in [Limb.LEFT_FOOT, Limb.RIGHT_FOOT] 
                          else self.constraints.min_ioi_hand)
                
                if ioi < min_ioi:
                    filtered.kept = False
                    filtered.removal_reason = f"Too fast for {limb.value}: {ioi:.0f}ms < {min_ioi:.0f}ms"
    ...
    def detect_impossible_patterns(
        ...
        for i, hit in enumerate(sorted_by_time):
            ...
            simultaneous = [h for h in sorted_by_time 
                          if abs(h.get('time', 0) - time) < 0.010]
            
            if len(simultaneous) > self.constraints.max_simultaneous:
                impossible.append((
                    i, i + len(simultaneous) - 1,
        ...
        for i, hit in enumerate(sorted_hits):
            ...
            if local_density > self.max_nps:
                ...
                if confidence < 0.8:
                    ...
                    is_on_beat = hit.get('is_backbeat', False) or \
                                 hit.get('beat_position', 0.5) % 1.0 < 0.1
...
def detect_sections(
    ...
    while current_start < max_time:
        ...
        section_hits = [h for h in sorted_hits 
                       if current_start <= h.get('time', 0) < section_end]
        ...
        sections.append({
            ...
            "unique_components": len(set(h.get('component', '') for h in section_hits)),
        })
...
def apply_difficulty_curve(
    ...
    for section in sections:
        ...
        section_hits = [h for h in hits 
                       if start <= h.get('time', 0) < end]
...
class DynamicDifficultyCurve:
    ...
    def _compute_fatigue_profile(
        ...
        for i, energy in enumerate(energy_profile):
            ...
            if energy > 0.7:
                ...
                current_fatigue -= sample_rate / (self.sustained_high_intensity_limit * self.recovery_rate)
    ...
    def apply_to_hits(
        ...
        # Compute the curve
        curve = self.compute_curve(hits, sections, total_duration)
