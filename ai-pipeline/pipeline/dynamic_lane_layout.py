...
class LaneDefinition:
    ...
    def matches(self, component: str) -> bool:
        ...
        return any(c.lower() in comp_lower or comp_lower in c.lower() 
...
class DynamicLaneLayoutBuilder:
    ...
    def _build_lanes(
        ...
        if ComponentCategory.KICK in category_components:
            ...
            if len(kick_comps) > 1 and not self._should_merge_kicks(kick_comps):
                ...
                for i, (comp, _) in enumerate(kick_comps[:2]):  # Max 2 kick lanes
                    lanes.append(LaneDefinition(
                        index=0, name=f"Kick {i+1}", short_name=f"K{i+1}",
                ...
                # Single kick lane
                lanes.append(LaneDefinition(
                    index=0, name="Kick", short_name="K",
                    category=ComponentCategory.KICK,
        ...
        if ComponentCategory.SNARE in category_components:
            ...
            if self.merge_ghost_notes and ComponentCategory.GHOST in category_components:
            ...
            lanes.append(LaneDefinition(
                index=0, name="Snare", short_name="S",
        ...
        if ComponentCategory.GHOST in category_components:
            ghost_comps = [c for c, _ in category_components[ComponentCategory.GHOST]]
            lanes.append(LaneDefinition(
                index=0, name="Ghost", short_name="G",
        ...
        if ComponentCategory.HIHAT in category_components:
            ...
            stick_comps = [c for c, _ in hat_comps if "pedal" not in c and "foot" not in c]
            
            if stick_comps:
                lanes.append(LaneDefinition(
                    index=0, name="Hi-Hat", short_name="HH",
            ...
            if pedal_comps and not self.merge_ghost_notes:
                ...
                lanes.append(LaneDefinition(
                    index=0, name="HH Pedal", short_name="HP",
                    category=ComponentCategory.HIHAT,
        ...
        if ComponentCategory.TOM in category_components:
            ...
            if self.merge_tom_varieties or len(tom_comps) <= 2:
                ...
                lanes.append(LaneDefinition(
                    index=0, name="Toms", short_name="T",
                ...
                generic_toms = [c for c, _ in tom_comps 
                               if c not in high_toms + mid_toms + low_toms]
                
                if high_toms:
                    lanes.append(LaneDefinition(
                        index=0, name="High Tom", short_name="HT",
                        category=ComponentCategory.TOM,
                ...
                if mid_toms or generic_toms:
                    lanes.append(LaneDefinition(
                        index=0, name="Mid Tom", short_name="MT",
                        category=ComponentCategory.TOM,
                ...
                if low_toms:
                    lanes.append(LaneDefinition(
                        index=0, name="Floor Tom", short_name="FT",
                        category=ComponentCategory.TOM,
                        components=low_toms,
                        color="#1A3440",
                        priority=5,
        ...
        if ComponentCategory.CYMBAL in category_components:
            ...
            if self.merge_similar_cymbals:
                ...
                if rides:
                    lanes.append(LaneDefinition(
                        index=0, name="Ride", short_name="RD",
                        category=ComponentCategory.CYMBAL,
                        components=rides,
                        color="#D4A84B",
                        priority=7,
                ...
                for comp, count in cymbal_comps:
                    if count > 5:  # Only if used meaningfully
                        lanes.append(LaneDefinition(
                            index=0, name=comp.replace("_", " ").title(),
        ...
        if ComponentCategory.PERCUSSION in category_components:
            perc_comps = [c for c, _ in category_components[ComponentCategory.PERCUSSION]]
            if perc_comps:
                lanes.append(LaneDefinition(
                    index=0, name="Percussion", short_name="PC",
    ...
    def _merge_lanes(
        ...
        while len(lanes) > self.max_lanes:
            ...
            lanes.sort(key=lambda l: l.priority)
            lowest = lanes[0]
    ...
    def _reorder_lanes(self, lanes: List[LaneDefinition]) -> List[LaneDefinition]:
        ...
        """
        category_order = [
            (ComponentCategory.HIHAT, lambda l: "pedal" in l.name.lower()),
            (ComponentCategory.SNARE, None),
            (ComponentCategory.GHOST, None),
            (ComponentCategory.TOM, lambda l: "high" in l.name.lower()),
            (ComponentCategory.KICK, None),
            (ComponentCategory.TOM, lambda l: "high" not in l.name.lower()),
            (ComponentCategory.HIHAT, lambda l: "pedal" not in l.name.lower()),
    ...
    def _create_default_layout(self) -> DynamicLaneLayout:
        ...
        default_lanes = [
            LaneDefinition(0, "Kick", "K", ComponentCategory.KICK, 
                          ["kick", "bass"], CATEGORY_COLORS[ComponentCategory.KICK], 10),
            LaneDefinition(1, "Snare", "S", ComponentCategory.SNARE,
                          ["snare", "ghost", "rim"], CATEGORY_COLORS[ComponentCategory.SNARE], 9),
            LaneDefinition(2, "Hi-Hat", "HH", ComponentCategory.HIHAT,
                          ["hihat"], CATEGORY_COLORS[ComponentCategory.HIHAT], 8),
            LaneDefinition(3, "Cymbal", "CY", ComponentCategory.CYMBAL,
                          ["crash", "ride"], CATEGORY_COLORS[ComponentCategory.CYMBAL], 7),
        ]
