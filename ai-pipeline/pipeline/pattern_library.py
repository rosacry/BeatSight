...
FUNK_PATTERNS = [
    DrumPattern(
        ...
        hits=[
            ...
            (2.25, "ghost", 0.4),
            (2.25, "hihat_closed", 0.5),
            (2.5, "kick", 0.9),
            (2.5, "hihat_closed", 0.6),
            (2.75, "ghost", 0.4),
            (2.75, "hihat_closed", 0.5),
            # Beat 4
            (3.0, "snare", 1.0),
            (3.0, "hihat_closed", 0.8),
...
class PatternLibrary:
    ...
    def match_pattern(
        ...
        for pattern in self.patterns.values():
            ...
            for start_idx, start_hit in enumerate(sorted_hits):
                ...
                window_hits = [
                    h for h in sorted_hits if start_time <= h.get("time", 0) < end_time
                ]
    ...
    def _components_match(self, pattern_comp: str, hit_comp: str) -> bool:
        ...
        if "tom" in pattern_comp and "tom" in hit_comp:
            return True
        if pattern_comp in ["crash", "ride", "cymbal"] and any(
            x in hit_comp for x in ["crash", "ride", "china", "splash"]
        ):
            return True
