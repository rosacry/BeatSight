...
class StorageClient:
    ...
    async def upload(self, source: Path, uri: str) -> str:
        ...
        logger.info(
            "Uploading to storage", uri=uri, scheme=scheme, size=source.stat().st_size
        )
    ...
    def _copy_local(self, source: Path, destination: Path) -> Path:
        ...
        logger.info(
            "Local copy complete", source=str(source), destination=str(destination)
        )
        return destination

    def generate_presigned_url(
        self, uri: str, expires_in: int = 3600, operation: str = "get"
    ) -> str:
        """
