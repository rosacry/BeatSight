...
class TransitionMatrix:
    ...
    base_transitions: np.ndarray = field(default_factory=lambda: np.array([
        ...
        [0.05, 0.10, 0.60, 0.15, 0.03, 0.02, 0.05],  # From GHOST (usually leads to snare)
    ], dtype=np.float64))
    ...
    min_ioi: Dict[DrumState, float] = field(default_factory=lambda: {
        DrumState.SILENCE: 0,
    ...
    def __post_init__(self):
        ...
        self.beat_modifiers[1] = self.beat_modifiers[3] = np.array([
            ...
            [0.8, 0.9, 1.6, 1.0, 1.0, 1.0, 0.9],  # Ghost → Snare boosted on backbeat
        ], dtype=np.float64)
...
class ViterbiDecoder:
    ...
    def decode(
        ...
        first_state = DrumState.from_component(first_event.get('component', ''))
...
def detect_swing_ratio(
    ...
    for i, ioi in enumerate(eighth_iois):
        ...
        cumulative_time = times[eighth_mask][:-1][i] if i < len(times[eighth_mask]) - 1 else 0
