...
class DrumSeparator:
    ...
    def __init__(
        ...
        if self.verbose:
            ...
            print(
                f"   Loading Demucs model '{self.model_name}' ({desc}) on {self.device}..."
            )
    ...
    def separate(
        ...
        if self.verbose:
            print(
                f"   Separation complete: {elapsed:.1f}s ({timing_info['realtime_factor']:.1f}x realtime)"
            )
    ...
    def separate_streaming(
        ...
        while position < total_samples:
            ...
            if self.verbose:
                elapsed = time.time() - start_time
                logger.info(
                    f"   Chunk {chunk_idx + 1}: {progress * 100:.0f}% ({elapsed:.1f}s elapsed)"
                )
