#!/usr/bin/env python3
"""Quick evaluation script for multi-label model."""
import torch
import numpy as np
import sys
...
def main():
    ...
    print(f'Epoch {ckpt["epoch"]}, best_val_f1: {ckpt["best_val_f1"]:.4f}')

    val_dataset = CachedMultiLabelDataset(
        data_dir='F:/datasets/prod_v5_multilabel/val',
        num_classes=12,
    ...
    # Use 20k for reasonably accurate results
    np.random.seed(42)
    indices = np.random.choice(len(val_dataset), 20000, replace=False)
    subset = torch.utils.data.Subset(val_dataset, indices)
    loader = DataLoader(subset, batch_size=512, num_workers=4, pin_memory=True)

    print(f'Evaluating on 20000 samples...')
