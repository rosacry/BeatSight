p# BeatSight ML Training Audit Report
...
Comprehensive audit of the BeatSight ML training pipeline completed. The codebase is well-structured with solid foundations. **90.06% balanced validation accuracy** has been achieved on the 12-class drum classifier.

### Key Achievements ✅

| Milestone | Status | Details |
|-----------|--------|---------|
| 90% Balanced Accuracy | ✅ Achieved | 90.06% at epoch 53 |
| Phase 1 Training | ✅ Complete | Using prod_v5_cleaned dataset |
| Phase 2 Training | 🔄 In Progress | Fine-tuning with SWA |
| Phase Transition Bug | ✅ Fixed | `--reset-scheduler` flag added |

### Key Findings

| Area | Status | Notes |
|------|--------|-------|
| Training Pipeline | ✅ Solid | 90.06% accuracy achieved |
| Data Loading | ✅ Well-optimized | 15.2M train / 1.69M val samples |
| Feature Cache | ✅ Excellent | float16 cache, sharded format |
| Model Architecture | ✅ Good | 7.1M params (v5-large) |
| Phase Transitions | ✅ Fixed | `--reset-scheduler` prevents LR bugs |
| Documentation | ✅ Updated | All training guides current |

---

## 🐛 Critical Bug Fix: Phase Transition LR Reset
...
- Optimizer's `initial_lr` retained Phase 1's value

This caused LR to **increase** (wrap around cosine) instead of decrease.

### The Solution
Added `--reset-scheduler` flag to `train_classifier.py` that:
1. Skips loading scheduler state from checkpoint
2. Resets optimizer's `initial_lr` and `lr` to match new `--lr` argument

### Usage
```bash
# When transitioning phases (different LR):
--resume-from runs/v5_phase1/checkpoints/best_checkpoint.pth --reset-scheduler

# When resuming within same phase (after crash):
--resume-from runs/v5_phase2/checkpoints/latest_checkpoint.pth  # NO --reset-scheduler
```
...
### Dataset: prod_v5_cleaned
| Metric | Value |
|--------|-------|
| Training Samples | 15,261,562 |
| Validation Samples | 1,695,730 |
| Classes | 12 |
| Class Imbalance | 49.7:1 (snare:china) |

### Training Progress
| Phase | Epochs | LR | Status | Best Accuracy |
|-------|--------|-----|--------|---------------|
| Phase 1 | 1-53 | 1e-4 | ✅ Complete | **90.06%** |
| Phase 2 | 54-84 | 1e-5 | 🔄 In Progress | - |
| Phase 3 | 85-104 | 1e-6 | ⬜ Pending | - |
