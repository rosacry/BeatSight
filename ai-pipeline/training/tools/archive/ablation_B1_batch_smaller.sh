#!/bin/bash
# B1: Smaller effective batch (512) - does smaller batch help generalization?
set -e
...
RUN_DIR="${BEATSIGHT_REPO_ROOT}/ai-pipeline/training/runs/ablation_B1_batch_smaller_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RUN_DIR"
cd "$BEATSIGHT_REPO_ROOT"
export NVIDIA_TF32_OVERRIDE=1
export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True,garbage_collection_threshold:0.8"
echo "======== B1: effective batch=512 (baseline: 1024) ========"
PYTHONPATH=ai-pipeline python ai-pipeline/training/train_classifier.py \
    ...
    --batch-size 256 --grad-accum-steps 2 --lr 0.0001 \
