#!/bin/bash
# E1: More data (10%) - does more data help?
set -e
...
RUN_DIR="${BEATSIGHT_REPO_ROOT}/ai-pipeline/training/runs/ablation_E1_more_data_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RUN_DIR"
cd "$BEATSIGHT_REPO_ROOT"
export NVIDIA_TF32_OVERRIDE=1
export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True,garbage_collection_threshold:0.8"
echo "======== E1: 10% data (baseline: 5%) ========"
PYTHONPATH=ai-pipeline python ai-pipeline/training/train_classifier.py \
    ...
    --batch-size 256 --grad-accum-steps 4 --lr 0.0001 \
    --num-workers 4 --val-num-workers 2 --prefetch-factor 2 --val-prefetch-factor 2 \
    --persistent-workers --pin-memory --amp-dtype bfloat16 \
    --train-fraction 0.10 --val-fraction 0.10 --subset-mode stratified --min-samples-per-class 50 \
