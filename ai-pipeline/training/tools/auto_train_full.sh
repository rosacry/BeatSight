...
#
# The script will:
#   1. Kill any existing training processes before starting
#   2. Automatically resume from latest checkpoint on crash
#   3. Retry indefinitely until training completes successfully
#   4. Wait 30 seconds between retries (in case of temporary issues)
#   5. Log all attempts to a log file
#   6. Send a notification when complete (if notify-send available)
#
# =============================================================================
...
# =============================================================================
# Kill Existing Training (Windows + Linux compatible)
# =============================================================================
# Ensure no other training is running before we start
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" || "$OSTYPE" == "win32" ]]; then
    # Windows: Use taskkill
    taskkill //F //IM python.exe 2>/dev/null && echo "[AUTO-TRAIN] Killed existing Python processes" || true
    sleep 2
else
    # Linux/Mac: Use pkill
    pkill -f "train_classifier.py" 2>/dev/null && echo "[AUTO-TRAIN] Killed existing training processes" || true
    sleep 1
fi
...
case "$TRAIN_MODE" in
    ...
    v5-full-cached-simple|v5-simple|17d-simple)
        ...
        ;;
    # =====================================================================
    # V5 LOCAL 3080 Ti (17d-local) - ULTRA-OPTIMIZED for 12GB VRAM consumer GPUs
    # Uses class weights (10x faster) instead of balanced sampling
    # ~12-18 minutes per epoch, ~4-6 hours for 20 epochs
    # =====================================================================
    v5-local|v5-3080|17d-local)
        ...
        ;;
    # =====================================================================
    # V5 LOCAL BALANCED (17d-balanced) - Maximum rare class accuracy (SLOW)
    # Uses balanced sampling for best quality on rare classes
    # ~45-75 minutes per epoch (5x slower than v5-local)
    # Use when: training final production model, rare class accuracy is critical
    # =====================================================================
    v5-local-balanced|17d-balanced)
        TRAIN_MODE="v5-local-balanced"
        RUN_DIR="$BEATSIGHT_RUN_CUTTING_EDGE/v5/local-balanced"
        ;;
    # V5 LOCAL BALANCED DISTILL (17e-local) - Born-Again Networks for local GPU
    v5-local-balanced-distill|17e-local)
        TRAIN_MODE="v5-local-balanced-distill"
        RUN_DIR="$BEATSIGHT_RUN_CUTTING_EDGE/v5/local-balanced-distill"
        ;;
    v5-full|17e)
        TRAIN_MODE="v5-full"
        RUN_DIR="$BEATSIGHT_RUN_CUTTING_EDGE/v5/full"
        ;;
    v5-self-distill-cached|17f)
        TRAIN_MODE="v5-self-distill-cached"
        RUN_DIR="$BEATSIGHT_RUN_CUTTING_EDGE/v5/self-distill-cached"
        ;;
    v5-self-distill|17g)
    ...
    *)
        ...
        echo "    v5-local            (17d-local)  - 🚀 LOCAL ULTRA: 10x faster (~15min/epoch)"
        echo "                           Class weights mode: 4-6hr for 20 epochs ⭐ RECOMMENDED"
        echo "    v5-local-balanced   (17d-balanced) - 🎯 LOCAL BALANCED: best rare class accuracy"
        echo "                           Balanced sampling: 45-75min/epoch (~25hr for 20 epochs)"
        ...
        echo "  🖥️  LOCAL GPU:        v5-local (fast) or v5-local-balanced (best quality)"
...
ENSEMBLE_PY
        ...
        v5-local)
            # =====================================================================
            # V5 LOCAL - ULTRA-OPTIMIZED for 3080 Ti + 9800X3D + NVMe + HDD dataset
            # =====================================================================
            # CRITICAL BOTTLENECK ANALYSIS (Dec 2024):
            # - Dataset on HDD (E: drive, 120 MB/s) but cache on NVMe (C: drive, 7000 MB/s)
            # - Balanced sampling causes random access across 225 shards = cache thrashing
            # - Current speed: 1.98s/batch (TERRIBLE - should be 0.1-0.3s)
            #
            # SOLUTION: Use class weights instead of balanced sampling for 10-20x speedup
            # - Balanced sampling: random access destroys locality (1.5-2s/batch)
            # - Class weights: sequential access, mmap stays cached (0.1-0.2s/batch)
            # - Quality trade-off: ~1-2% on rare classes, but 10-20x faster training
            #
            # VRAM-optimized settings for 12GB 3080 Ti:
            # → Batch 384 (safe for 12GB with all augmentations)
            # → Gradient accumulation 4 (effective batch = 1536)
            # → float16 + gradient checkpointing + channels_last
            # → 4 workers with prefetch 4 (optimal for sequential access)
            #
            # Expected performance AFTER optimization:
            # → ~0.15-0.25s per batch (was 1.98s - 10x improvement!)
            # → ~12-18 minutes per epoch (was 75+ min)
            # → ~4-6 hours for 20 epochs (was 25+ hours)
            # =====================================================================
            
            log "🖥️  Starting V5 LOCAL training (ULTRA-OPTIMIZED for 3080 Ti + 9800X3D)..."
            log "   💾 VRAM: 12GB 3080 Ti with gradient checkpointing"
            log "   🧠 CPU: 9800X3D - excellent single-thread for data loading"
            log "   💨 Storage: NVMe cache (7000 MB/s) + HDD dataset (120 MB/s)"
            log ""
            log "   🚀 ULTRA-OPTIMIZED SETTINGS (v4 - stable):"
            log "      → Batch size: 384 (with grad checkpoint for stability)"
            log "      → Gradient accumulation: 4 (effective batch = 1536)"
            log "      → Samples per epoch: 2M (5,208 batches vs 38K full)"
            log "      → float16 precision + TF32 matmul (Ampere optimal)"
            log "      → Gradient checkpointing enabled (required for 12GB)"
            log "      → 2 workers + prefetch 2 (stable on Windows)"
            log "      → NO persistent workers (fixes resume deadlock)"
            log "      → Class weights instead of balanced sampling (10x speedup)"
            log "      → CUDA memory optimizations enabled"
            log ""
            log "   🔴 WARNING: This mode achieves ONLY ~41% accuracy!"
            log "      → 19/21 classes get 0% accuracy (class weights too weak)"
            log "      → For 95%+ accuracy, use: v5-local-balanced"
            log ""
            log "   ⚠️  IMPORTANT: Using class weights instead of balanced sampling!"
            log "      → This trades MASSIVE rare class accuracy for 10x training speed"
            log "      → For best quality, use v5-local-balanced mode"
            log ""
            log "   ⏱️  Expected time:"
            log "      → ~1.6-1.8s per batch"
            log "      → ~2.5 hours per epoch (2M samples)"
            log "      → ~50 hours for 20 epochs"
            log ""
            log "   💰 Cost: FREE (just electricity!)"
            log ""
            export WANDB_RUN_GROUP=v5_local_ultra
            
            # =================================================================
            # CUDA & PyTorch Performance Optimizations (CRITICAL)
            # =================================================================
            # Enable TF32 for 3x faster matmul on Ampere (3080 Ti)
            export NVIDIA_TF32_OVERRIDE=1
            
            # CUDA memory allocator: use expandable segments to reduce fragmentation
            # This prevents OOM from memory fragmentation during long training runs
            export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True,garbage_collection_threshold:0.8"
            
            # Disable NCCL (single-GPU training, avoid multi-GPU overhead)
            export NCCL_P2P_DISABLE=1
            
            # cuDNN optimizations for consistent input sizes
            ...
            # =================================================================
            # DataLoader Settings (I/O Optimized)
            # =================================================================
            # CRITICAL FIX: Removed balanced sampling for 10x speedup!
            # With HDD dataset + mmap cache, sequential access is essential
            # Using class weights instead to handle imbalance (slightly less effective but 10x faster)
            # OPTIMIZED (Dec 2024): 2 workers + prefetch 4 gives ~30% faster training
            # The old "0 workers fastest" was tested without proper GPU utilization monitoring
            # With 0 workers: GPU starves at 80% utilization (1.61s/batch)
            # With 2 workers: GPU reaches 95%+ utilization (~1.0-1.2s/batch expected)
            # NOTE: More than 2 workers on Windows can cause GIL contention issues
            LOCAL_BATCH_SIZE="384"         # Keep safe size with grad checkpoint (9.3GB VRAM used)
            LOCAL_GRAD_ACCUM="4"           # 384 * 4 = 1536 effective batch
            LOCAL_AMP_DTYPE="float16"      # Best for Ampere architecture
            LOCAL_NUM_WORKERS="2"          # 2 workers optimal for Windows + NVMe + 9800X3D
            LOCAL_VAL_WORKERS="2"          # Match for validation
            LOCAL_PREFETCH="4"             # NVMe can easily prefetch 4 batches (7000 MB/s)
            ...
            LOCAL_FOCAL_FLAGS="--focal-loss --focal-gamma 2.0"
            LOCAL_EMA_FLAGS="--use-ema --ema-decay 0.999"
            LOCAL_LABEL_SMOOTHING="--label-smoothing 0.1"
            
            # SPEED FIX: Use class weights instead of balanced sampling (10x faster!)
            # Balanced sampling destroys I/O locality with mmap cache
            # Class weights achieve similar rebalancing effect with sequential access
            # Trade-off: ~1-2% less accuracy on rare classes, but 10x faster training
            LOCAL_BALANCED_SAMPLING="--class-weights effective --max-class-weight 15.0"
            
            # Removed SWA (conflicts with warm restarts scheduler)
            LOCAL_SWA_FLAGS=""
            # OPTIMIZED: Removed FMix (already have Mixup+CutMix, 3 mixing methods is excessive)
            LOCAL_FMIX_FLAGS=""
            # OPTIMIZED: Increased patience to 25 (was 15 - too short for warm restart recovery)
            LOCAL_EARLY_STOPPING="--early-stopping --early-stopping-patience 25 --early-stopping-min-delta 0.001 --early-stopping-warmup 5"
            # Removed FMix (already have Mixup+CutMix, 3 mixing methods is excessive)
            LOCAL_FMIX_FLAGS=""
            
            # Early stopping with longer patience for warm restarts
            LOCAL_EARLY_STOPPING="--early-stopping --early-stopping-patience 25 --early-stopping-min-delta 0.001 --early-stopping-warmup 5"
            
            # Gradient checkpointing - required for batch 384 + all augmentations on 12GB
            ...
            # =================================================================
            # Epoch Size Configuration (SPEED OPTIMIZATION)
            # =================================================================
            # Full dataset: 14.6M samples = 38,233 batches = ~16 hours per epoch!
            # Solution: Limit samples per epoch for faster iteration
            # 2M samples = 5,208 batches = ~1-2 hours per epoch at 1s/batch
            LOCAL_SAMPLES_PER_EPOCH="2000000"
            
            # =================================================================
            # Launch Training
            # =================================================================
            # NOTE: --persistent-workers REMOVED to fix Windows deadlock on checkpoint resume
            PYTHONPATH=ai-pipeline python ai-pipeline/training/train_classifier.py \
              --dataset "${BEATSIGHT_DATASET_DIR}" \
              --labels-cache-dir "${BEATSIGHT_DATA_ROOT}/dataset_index" \
              --feature-cache-dir "${BEATSIGHT_CACHE_DIR}" \
              ...
              --samples-per-epoch ${LOCAL_SAMPLES_PER_EPOCH} \
              --lr 0.002 \
              ${LOCAL_MODEL_FLAGS} \
              ${LOCAL_MIXUP_FLAGS} \
            ...
            log ""
            log "🖥️  V5 LOCAL ULTRA training complete!"
            ;;
        
        v5-local-balanced)
            # =====================================================================
            # V5 LOCAL BALANCED - MAXIMUM QUALITY MODE (REQUIRED FOR 95%+ ACCURACY)
            # =====================================================================
            # Your v5-local model FAILED because class weights alone can't fix 630x imbalance
            # Per-class analysis shows: 19/21 classes at 0% accuracy!
            # 
            # BALANCED SAMPLING IS MANDATORY for learning all classes.
            # Yes it's slower. But a fast broken model is worthless.
            #
            # This mode will achieve:
            # - 95%+ on common classes (kick, snare, hihat, crash, ride, toms)
            # - 85-90% on medium classes (rimshot, cross-stick, splash)  
            # - 70-80% on rare classes (cymbal_choke, hihat_closed edge cases)
            # =====================================================================
            
            log "🎯 Starting V5 LOCAL BALANCED training (MAXIMUM QUALITY)..."
            log "   💾 VRAM: 12GB 3080 Ti with gradient checkpointing"
            log "   🧠 CPU: 9800X3D - excellent single-thread for data loading"
            log "   💨 Storage: NVMe cache (7000 MB/s)"
            log ""
            log "   🔴 WHY BALANCED SAMPLING IS REQUIRED:"
            log "      → Your v5-local model has 19/21 classes at 0% accuracy!"
            log "      → Class weights (15x max) can't fix 630x imbalance"
            log "      → Balanced sampling ensures ALL classes are learned"
            log ""
            log "   📊 QUALITY-OPTIMIZED Settings:"
            log "      → Batch size: 384 (stable with all augmentations)"
            log "      → Gradient accumulation: 4 (effective batch = 1536)"
            log "      → 4 workers + prefetch 4 (good I/O parallelism)"
            log "      → UNIFORM balanced sampling (equal class probability)"
            log "      → Higher focal gamma (3.0) for hard examples"
            log ""
            log "   ⏱️  Expected time:"
            log "      → ~2.2s per batch (random I/O pattern)"
            log "      → ~17-18 hours per epoch (training)"
            log "      → ~6 min validation (20% subset)"
            log "      → Early stopping likely at epoch 30-50"
            log "      → Total estimate: 15-25 days (NOT 100 epochs)"
            log ""
            log "   🚀 Speed optimizations applied:"
            log "      → val-fraction=0.2 (80% less validation time)"
            log "      → checkpoint-every-batches=5000 (less I/O)"
            log "      → bfloat16 (better Ampere support)"
            log ""
            export WANDB_RUN_GROUP=v5_local_balanced_quality
            ...
            # =================================================================
            # OPTIMIZED for 9800X3D + NVMe + Balanced Sampling (Windows)
            # =================================================================
            # Key insight: Balanced sampling = random I/O across 225 shards
            # On Windows, more workers can cause GIL contention with random access
            # 
            # Tested results:
            # - 4 workers: 1.68s/it ✅ FASTEST
            # - 8 workers: 2.40s/it ❌ Slower due to contention
            # 
            # Keep batch 512 for fewer iterations, but use fewer workers
            # =================================================================
            
            # Larger batch = fewer gradient syncs, better GPU efficiency
            BAL_BATCH_SIZE="512"           # Larger batch = fewer iterations per epoch
            BAL_GRAD_ACCUM="3"             # 512 * 3 = 1536 effective batch
            BAL_AMP_DTYPE="float16"
            
            # FEWER workers on Windows with balanced sampling (less GIL contention)
            BAL_NUM_WORKERS="4"            # 4 workers was faster than 8 on Windows
            BAL_VAL_WORKERS="2"
            ...
            # The reduce-overhead mode uses CUDA graphs which conflict with grad checkpointing
            BAL_TORCH_COMPILE=""
            
            # Checkpoint detection
            BAL_OUTPUT_DIR="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced"
            LOCAL_CHECKPOINT="${BAL_OUTPUT_DIR}/checkpoints/latest_checkpoint.pth"
            
            resume_from_flag=""
            if [[ -f "$LOCAL_CHECKPOINT" ]]; then
                log "   📂 Found checkpoint: $LOCAL_CHECKPOINT"
                resume_from_flag="--resume-from $LOCAL_CHECKPOINT"
            fi
            
            # =====================================================================
            # SPEED OPTIMIZATIONS (with minimal quality impact):
            # ---------------------------------------------------------------------
            # 1. --val-fraction 0.2: Validate on 20% of val set (~320K samples)
            #    → Saves ~25 min per epoch, statistically still representative
            ...
            PYTHONPATH=ai-pipeline python ai-pipeline/training/train_classifier.py \
              ...
              --feature-cache-dir "${BEATSIGHT_CACHE_DIR}" \
              --device cuda \
              --num-workers ${BAL_NUM_WORKERS} --val-num-workers ${BAL_VAL_WORKERS} --prefetch-factor ${BAL_PREFETCH} --val-prefetch-factor ${BAL_VAL_PREFETCH} \
              --persistent-workers \
              --pin-memory --amp-dtype bfloat16 \
            ...
            log ""
            log "🖥️  V5 LOCAL BALANCED training complete!"
            ;;
        
        # =====================================================================
        # V5 LOCAL BALANCED DISTILL (17e-local) - Born-Again Networks for LOCAL GPU
        # =====================================================================
        # Self-distillation using v5-local-balanced as teacher
        # Optimized for 3080 Ti (12GB VRAM) + 9800X3D
        # Reference: "Born-Again Neural Networks" (Furlanello et al., ICML 2018)
        # =====================================================================
        
        v5-local-balanced-distill)
            log "🔄 Starting V5 LOCAL BALANCED SELF-DISTILLATION (Born-Again Networks)..."
            log "   Optimized for: RTX 3080 Ti (12GB) + 9800X3D + NVMe cache"
            log "   Using v5-local-balanced model as teacher, training identical student..."
            log "   Expected improvement: +1-2% from dark knowledge transfer..."
            log ""
            log "   📊 Settings (matching v5-local-balanced):"
            log "      → Batch size: 512 (same as teacher training)"
            log "      → Gradient accumulation: 3 (effective batch = 1536)"
            log "      → UNIFORM balanced sampling (essential for rare classes)"
            log "      → Knowledge distillation from teacher soft labels"
            log ""
            export WANDB_RUN_GROUP=v5_local_balanced_distill_auto
            
            # Environment for local GPU optimization
            export NVIDIA_TF32_OVERRIDE=1
            export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True,garbage_collection_threshold:0.8"
            export CUDNN_BENCHMARK=1
            
            # Check for teacher model from v5-local-balanced
            TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier_ema.pth"
            if [ ! -f "$TEACHER_MODEL" ]; then
                TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier.pth"
            fi
            
            if [ ! -f "$TEACHER_MODEL" ]; then
                log "ERROR: Teacher model not found."
                log "   You must complete v5-local-balanced training first!"
                log "   Expected: ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier.pth"
                log ""
                log "   Run: bash ai-pipeline/training/tools/auto_train.sh v5-local-balanced"
                exit 1
            fi
            ...
            # Same settings as v5-local-balanced but with distillation
            DISTILL_BATCH_SIZE="512"
            DISTILL_GRAD_ACCUM="3"
            DISTILL_NUM_WORKERS="4"
            DISTILL_VAL_WORKERS="2"
            DISTILL_PREFETCH="4"
            
            DISTILL_MODEL_FLAGS="--model-version v5 --v5-size large --drop-path-rate 0.15"
            DISTILL_MIXUP_FLAGS="--mixup-alpha 0.2 --cutmix-alpha 0.5 --mixup-prob 0.5"
            DISTILL_SPECAUGMENT_FLAGS="--specaugment drum"
            DISTILL_FOCAL_FLAGS="--focal-loss --focal-gamma 3.0"
            DISTILL_EMA_FLAGS="--use-ema --ema-decay 0.9995"
            DISTILL_LABEL_SMOOTHING="--label-smoothing 0.1"
            DISTILL_BALANCED_SAMPLING="--balanced-sampling --sampling-strategy uniform --class-weights none"
            DISTILL_EARLY_STOPPING="--early-stopping --early-stopping-patience 25 --early-stopping-min-delta 0.001 --early-stopping-warmup 5"
            DISTILL_GRAD_CHECKPOINT="--gradient-checkpointing"
            
            # Knowledge distillation flags
            DISTILL_KD_FLAGS="--distill-from ${TEACHER_MODEL} --distill-alpha 0.7 --distill-temperature 4.0"
            
            # Output directory
            DISTILL_OUTPUT_DIR="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced-distill"
            LOCAL_CHECKPOINT="${DISTILL_OUTPUT_DIR}/checkpoints/latest_checkpoint.pth"
            ...
            log "   ⏱️  Expected time:"
            log "      → ~3-4 hours per epoch (matching teacher)"
            log "      → Early stopping likely at epoch 30-50"
            log "      → Total estimate: 4-7 days"
            log ""
            ...
            PYTHONPATH=ai-pipeline python ai-pipeline/training/train_classifier.py \
              ...
              --feature-cache-dir "${BEATSIGHT_CACHE_DIR}" \
              --device cuda \
              --num-workers ${DISTILL_NUM_WORKERS} --val-num-workers ${DISTILL_VAL_WORKERS} --prefetch-factor ${DISTILL_PREFETCH} --val-prefetch-factor 2 \
              --persistent-workers \
              --pin-memory --amp-dtype bfloat16 \
              --cache-warmup --cache-warmup-samples 500000 \
              --epochs 100 \
              --batch-size ${DISTILL_BATCH_SIZE} \
              --grad-accum-steps ${DISTILL_GRAD_ACCUM} \
              --lr 0.001 \
              ${DISTILL_MODEL_FLAGS} \
              ${DISTILL_MIXUP_FLAGS} \
              ${DISTILL_SPECAUGMENT_FLAGS} \
              ${DISTILL_FOCAL_FLAGS} \
              ${DISTILL_EMA_FLAGS} \
              ${DISTILL_LABEL_SMOOTHING} \
              ${DISTILL_BALANCED_SAMPLING} \
              ${DISTILL_EARLY_STOPPING} \
              ${DISTILL_GRAD_CHECKPOINT} \
              ${DISTILL_KD_FLAGS} \
              --scheduler cosine_warm_restarts \
              --warm-restart-t0 20 \
              --warm-restart-mult 2 \
            ...
            log ""
            log "🖥️  V5 LOCAL BALANCED DISTILL training complete!"
            log "📁 Best model: ${DISTILL_OUTPUT_DIR}/best_drum_classifier.pth"
            log "🚀 This is your PRODUCTION model - deploy this!"
            ;;
        ...
        # =====================================================================
        # V5 SELF-DISTILLATION CACHED (17f) - Born-Again Networks (Speed Optimized)
        # Train V5 again using the first V5's predictions as teacher
        # Reference: "Born-Again Neural Networks" (Furlanello et al., ICML 2018)
        # NOTE: Matches v5-full-cached settings for speed + uses cached teacher model
        # =====================================================================
        
        v5-self-distill-cached)
            log "🔄 Starting V5 SELF-DISTILLATION CACHED training (Born-Again Networks)..."
            log "   200 epochs (matching v5-full-cached speed optimizations)"
            log "   Using v5-full-cached model as teacher, training identical student..."
            log "   + Technique Heads: flam, roll, choke, ghost, accent detection"
            log "   Expected improvement: +1-2% from dark knowledge transfer..."
            log "   Includes: Attentive Statistics Pooling (Option A enhancement)..."
            log ""
            log "   🚀 SPEED OPTIMIZATIONS (matching v5-full-cached):"
            log "      → Batch size 2048 (optimal for throughput)"
            log "      → torch.compile max-autotune (~15% speedup after warmup)"
            log "      → AWP frequency 8 (half overhead)"
            log "      → 200 epochs (early stopping catches optimal anyway)"
            log "      → No SAM, No R-Drop (saves forward passes)"
            log "      → No ghost/waveform/accent-tap augmentation (uses cache)"
            log "   🔥 Estimated time: ~20-25hr on H100 80GB (same as v5-full-cached)"
            log ""
            export WANDB_RUN_GROUP=v5_self_distill_cached_auto
            
            # Check for teacher model - search in order of quality/recency
            # Priority: local-balanced (current best) > self-distill-cached > full-cached > full
            TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier_ema.pth"
            if [ ! -f "$TEACHER_MODEL" ]; then
                TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier.pth"
            fi
            if [ ! -f "$TEACHER_MODEL" ]; then
                TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full-cached/best_drum_classifier_ema.pth"
            fi
            if [ ! -f "$TEACHER_MODEL" ]; then
                TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full-cached/best_drum_classifier.pth"
            fi
            # Fallback to v5-full if cached version not available
            if [ ! -f "$TEACHER_MODEL" ]; then
            ...
            if [ ! -f "$TEACHER_MODEL" ]; then
                log "ERROR: Teacher model not found."
                log "   Run one of these first:"
                log "     - v5-local-balanced (17d-balanced) - LOCAL TRAINING WITH BALANCED SAMPLING"
                log "     - v5-full-cached (17d)"
                log "     - v5-full (17e)"
                log ""
                log "   Expected locations:"
                log "     - ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier.pth"
                log "     - ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full-cached/best_drum_classifier.pth"
                exit 1
            fi
            ...
            # Detect cloud GPU for optimizations (matching v5-full-cached)
            IS_CLOUD_GPU=false
            CLOUD_AMP_DTYPE="float16"
            CLOUD_BATCH_SIZE="1024"
            CLOUD_COMPILE_FLAGS=""
            ...
            # AWP every 8 batches (matching v5-full-cached)
            V5_AWP_FLAGS_TURBO="--use-awp --awp-lr 0.01 --awp-eps 0.01 --awp-start-epoch 5 --awp-freq 8"
            ...
            PYTHONPATH=ai-pipeline python ai-pipeline/training/train_classifier.py \
              ...
              --output "${BEATSIGHT_RUN_CUTTING_EDGE}/v5/self-distill-cached" \
              --seed 2024 \
              --checkpoint-every 10 \
              --wandb-project beatsight-v5 \
              $resume_flag
            ...
            log ""
            log "🔄 V5 Self-Distillation (CACHED) complete!"
            log "This model learned from both ground truth labels AND the teacher's TTA-ensemble predictions."
            log "The 'dark knowledge' from TTA-smoothed soft predictions provides +1-2% improvement."
            log ""
            log "📁 Final model saved to: ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/self-distill-cached/"
            log "🚀 This is your PRODUCTION model - deploy this one!"
            ;;
        
        # =====================================================================
        # V5 SELF-DISTILLATION (17g) - Born-Again Networks (FULL - with augmentation)
        # Train V5 again using the first V5's predictions as teacher
        # Reference: "Born-Again Neural Networks" (Furlanello et al., ICML 2018)
        # NOTE: Matches v5-full settings - includes ghost/waveform augmentation
        # =====================================================================
        
        v5-self-distill)
            log "🔄 Starting V5 SELF-DISTILLATION training (Born-Again Networks, FULL)..."
            log "   ⚠️  WARNING: This mode uses ghost/waveform augmentation which DISABLES caching!"
            log "   ⚠️  Estimated time: ~400-500 hours on H100 80GB"
            log "   ⚠️  Consider using v5-self-distill-cached (17f) for ~20-25hr training."
            log ""
            log "   300 epochs for maximum convergence (matching v5-full)"
            log "   Using v5-full model as teacher, training identical student..."
            log "   + Technique Heads: flam, roll, choke, ghost, accent detection"
            log "   + Ghost Note Augmentation (bypasses cache ~25% of batches)..."
            log "   + Accent-Tap Augmentation (+2-5% dynamics differentiation)..."
            log "   Expected improvement: +1-2% from dark knowledge + better ghost detection..."
            log "   Includes: Attentive Statistics Pooling (Option A enhancement)..."
            log ""
            export WANDB_RUN_GROUP=v5_self_distill_auto
            
            # Check for teacher model - prioritize v5-full (with augmentation)
            TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full/best_drum_classifier_ema.pth"
            if [ ! -f "$TEACHER_MODEL" ]; then
                TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full/best_drum_classifier.pth"
            fi
            # Fallback to v5-full-cached if full version not available
            if [ ! -f "$TEACHER_MODEL" ]; then
                TEACHER_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full-cached/best_drum_classifier_ema.pth"
            fi
            if [ ! -f "$TEACHER_MODEL" ]; then
            ...
            if [ ! -f "$TEACHER_MODEL" ]; then
                log "ERROR: Teacher model not found. Run v5-full (17e) or v5-full-cached (17d) first."
                log "   Expected: ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full/best_drum_classifier_ema.pth"
                log "        or: ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/full-cached/best_drum_classifier.pth"
                exit 1
            fi
            ...
            # Detect cloud GPU for optimizations (matching v5-full)
            IS_CLOUD_GPU=false
            CLOUD_AMP_DTYPE="float16"
            CLOUD_BATCH_SIZE="384"
            CLOUD_COMPILE_FLAGS=""
            ...
            log ""
            log "🔄 V5 Self-Distillation (FULL) complete!"
            log "This model learned from both ground truth labels AND the teacher's TTA-ensemble predictions."
            log "The 'dark knowledge' + ghost/waveform augmentation provides maximum ghost note detection."
            log ""
            log "📁 Final model saved to: ${BEATSIGHT_RUN_CUTTING_EDGE}/v5/self-distill/"
            log "🚀 This is your ULTIMATE PRODUCTION model!"
            ;;
        
        # =====================================================================
        # V5 ENSEMBLE TRAINING (17h) - Train 3 models with different seeds
        ...
        multilabel-finetune)
            ...
            log "   Uses V5 checkpoint as backbone initialization..."
            log "   Faster convergence + better features from balanced training..."
            ...
            # Priority: local-balanced (current best) > self-distill-cached > full-cached > full (EMA preferred)
            PRETRAINED_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier_ema.pth"
            if [ ! -f "$PRETRAINED_MODEL" ]; then
                PRETRAINED_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/local-balanced/best_drum_classifier.pth"
            fi
            if [ ! -f "$PRETRAINED_MODEL" ]; then
                PRETRAINED_MODEL="${BEATSIGHT_RUN_CUTTING_EDGE}/v5/self-distill-cached/best_drum_classifier_ema.pth"
            fi
