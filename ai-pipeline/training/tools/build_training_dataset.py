...
# CANONICAL component list - alphabetically sorted for consistent label indices.
# This MUST match the order expected by inference models (ml_drum_classifier.py).
# DO NOT reorder! Adding new classes should maintain alphabetical order.
#
# NOTE: Multi-instance instruments (toms, crashes, chinas, splashes) are stored
# as single classes. Pitch differentiation (high/mid/low) is handled via
# post-processing since relative pitch is song-specific.
#
# Redundant articulations are also merged (via excluded_classes.py):
#   - snare_center → snare (same drum, subtle difference)
#   - snare_cross_stick → cross_stick (same technique)
#   - snare_rimshot, rimshot → snare (detected via RimshotDetector post-processor)
#   - hihat_foot_splash → hihat_pedal (foot technique)
#   - hihat_splash → hihat_open (sounds similar to open hihat)
#
# Excluded classes (via excluded_classes.py):
#   - aux_percussion (inconsistent grab-bag of misc sounds)
#   - cymbal_choke (detected via post-processing sustain analysis)
#   - shaker, tambourine, drum_mix, cowbell, clap
CANONICAL_COMPONENTS = [
    ...
    "hihat_pedal",    # Foot sounds (pedal, foot_splash)
    "kick",
    "ride_bell",
    "ride_bow",
    "snare",          # All snare hits (merged snare_center, rimshot)
    "splash",         # All splash cymbals (pitch-ranked in post-processing)
    "tom",            # All toms merged (pitch-ranked in post-processing)
]

# Labels that should be merged into a canonical class during dataset building.
# NOTE: Most remapping is now handled by excluded_classes.py. This map is kept
...
def assign_split(session_id: str, *, val_ratio: float) -> str:
    """Legacy session-based split assignment. Use assign_split_stratified for balanced splits."""
    digest = hashlib.sha1(session_id.encode("utf-8")).digest()
    value = int.from_bytes(digest[:8], "big") / 2**64
    return "val" if value < val_ratio else "train"


class StratifiedSplitAssigner:
    """
    Assigns train/val splits with per-class stratification.
    
    Unlike session-based hashing which can cause severe imbalance for rare classes,
    this assigner maintains the target val_ratio independently for EACH class.
    
    This ensures that validation metrics are meaningful - if a class has 1000 samples,
    exactly ~100 will go to val (with val_ratio=0.1), regardless of session distribution.
    """
    
    def __init__(self, val_ratio: float, seed: int = 42):
        self.val_ratio = val_ratio
        self.seed = seed
        # Track counts per class to maintain stratification
        self._class_counts: Counter[str] = Counter()
        self._class_val_counts: Counter[str] = Counter()
        # Use deterministic RNG seeded per (class, count) for reproducibility
        self._rng = {}  # class -> Random instance
    
    def _get_rng(self, label: str):
        if label not in self._rng:
            import random
            # Seed based on class name for deterministic, reproducible splits
            class_seed = self.seed + hash(label) % (2**31)
            self._rng[label] = random.Random(class_seed)
        return self._rng[label]
    
    def assign(self, label: str, clip_id: str) -> str:
        """
        Assign a clip to train or val, maintaining per-class val_ratio.
        
        Args:
            label: The class label (used for stratification)
            clip_id: Unique clip identifier (used for deterministic assignment)
            
        Returns:
            "train" or "val"
        """
        # Current state for this class
        total = self._class_counts[label]
        val_count = self._class_val_counts[label]
        
        # Target val count for current total
        target_val = total * self.val_ratio
        
        # Decide based on whether we're under target
        # Use deterministic randomness based on clip_id for reproducibility
        rng = self._get_rng(label)
        
        # If we're significantly under target, prefer val; if over, prefer train
        # Add small random jitter to avoid exact boundaries creating patterns
        current_ratio = val_count / total if total > 0 else 0.0
        threshold = self.val_ratio + rng.uniform(-0.02, 0.02)  # Small jitter
        ...
        if split == "val":
            self._class_val_counts[label] += 1
        
        return split
    
    def get_stats(self) -> Dict[str, Dict[str, float]]:
        """Return per-class split statistics."""
        stats = {}
        for label in self._class_counts:
            total = self._class_counts[label]
            val_count = self._class_val_counts[label]
            actual_ratio = val_count / total if total > 0 else 0.0
            stats[label] = {
                "total": total,
                "train": total - val_count,
                "val": val_count,
                "actual_val_ratio": actual_ratio,
                "target_val_ratio": self.val_ratio,
                "ratio_error": abs(actual_ratio - self.val_ratio),
            }
        return stats
...
def build_dataset(
    *,
    ...
    if clip_fanout > 8:
        clip_fanout = 8
    stats = ExportStats()
    # Initialize components with canonical alphabetical ordering for consistent label indices
    components: List[str] = list(CANONICAL_COMPONENTS)
    component_index: Dict[str, int] = {label: idx for idx, label in enumerate(CANONICAL_COMPONENTS)}
    ...
    # Use stratified split assigner for balanced per-class train/val ratios
    split_assigner = StratifiedSplitAssigner(val_ratio=val_ratio, seed=42)
    ...
    if resume:
        ...
        if components_path.exists():
            ...
            if isinstance(existing_components, Mapping):
                ...
                # Validate existing components match canonical ordering
                if isinstance(raw_components, list) and isinstance(raw_index, Mapping):
                    existing_list = [str(item) for item in raw_components]
                    if existing_list != components:
                        logger.print(
                            f"[WARN] Existing components.json ordering differs from CANONICAL_COMPONENTS!"
                        )
                        logger.print(f"  Existing: {existing_list}")
                        logger.print(f"  Canonical: {components}")
                        logger.print("  Using CANONICAL_COMPONENTS ordering (existing data may have wrong indices).")
                # Keep canonical ordering - do NOT override from existing file
                # The canonical ordering is now the source of truth

        if per_label_counts and components:
            for label in per_label_counts:
                if label not in component_index:
                    logger.print(
                        f"[WARN] Previously seen label '{label}' not in CANONICAL_COMPONENTS - ignoring."
                    )
    ...
    def build_metadata_dict() -> Dict[str, object]:
        ...
        metadata = {
            ...
            "stratified_split_stats": split_assigner.get_stats(),  # Per-class train/val balance
    ...
    with progress_context as progress_tracker, live_display as live_display_manager:
        ...
        def report_progress() -> None:
            ...
            if live_enabled:
                ...
                heal_status = ""
                if missing_clip_units_total and missing_clip_units_remaining < missing_clip_units_total:
                    healed_so_far = missing_clip_units_total - missing_clip_units_remaining
                    heal_status = f" | healed: {healed_so_far:,}/{missing_clip_units_total:,}"
                eta_str = _format_eta(current_overall_eta) if current_overall_eta else "-"
                logger.print(
                    f"[{stats.total_events:,}] clips: {clip_summary} | "
                    f"train: {split_counts['train']:,} | val: {split_counts['val']:,} | "
                    f"skipped: {stats.skipped_total:,} | ETA: {eta_str}{heal_status}"
                )
                if not first_progress_message_sent:
                    first_progress_message_sent = True
        ...
        for event in iter_manifest_events(manifest_path):
            ...
            # NOTE: Split is now assigned PER-COMPONENT below using stratified assigner
            # to ensure each class maintains the target val_ratio independently.
            ...
            for component_entry in components_data:
                ...
                # Validate label is in canonical component list - reject unknown labels
                if label not in component_index:
                    logger.log(
                        f"[WARN] Unknown label '{label}' not in CANONICAL_COMPONENTS - skipping. "
                        f"Add to CANONICAL_COMPONENTS if this is a valid drum class."
                    )
                    stats.skipped_excluded_class += 1
                    continue
                ...
                # Stratified split: each class independently maintains val_ratio
                split = split_assigner.assign(label, clip_id)
