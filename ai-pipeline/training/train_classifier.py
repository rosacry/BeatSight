...
class _DeterministicWeightedSampler(torch.utils.data.Sampler[int]):
    """Deterministic replacement sampling with optional offset.

    This is designed to make mid-epoch resume practical:
    - For a given (seed, weights, num_samples), the sampled index sequence is stable.
    - When resuming mid-epoch, we slice the sequence at a sample offset so we do not
      re-run earlier batches.

    Note: This sampler intentionally mirrors the distribution of WeightedRandomSampler
    ...
    def __init__(
        ...
        *,
        ...
        if start_sample_index > num_samples:
            raise ValueError("start_sample_index cannot exceed num_samples")
        self._weights = weights
        self._num_samples = int(num_samples)
        self._seed = int(seed)
        self._start = int(start_sample_index)

    def __iter__(self):
        gen = torch.Generator(device="cpu")
        # manual_seed expects a 64-bit unsigned-ish value; keep it stable.
        gen.manual_seed(self._seed % (2**63 - 1))
        # WeightedRandomSampler uses torch.multinomial under the hood.
        samples = torch.multinomial(self._weights, self._num_samples, replacement=True, generator=gen)
        if self._start:
            samples = samples[self._start :]
        for idx in samples:
            yield int(idx)

    def __len__(self) -> int:
        return self._num_samples - self._start
...
class DrumSampleDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        # Support DUAL cache readers for combined train/val mappings
        self._consolidated_reader: Optional[Any] = None
        self._consolidated_reader_alt: Optional[Any] = None  # Second cache reader (for val cache when training)
        self._cache_base_dir: Optional[Path] = None  # Base cache dir (parent of train/val)
        ...
        if self.cache_dir is not None and HAS_CONSOLIDATED_CACHE:
            ...
            if consolidated_manifest.exists():
                try:
                    ...
                    # Store base dir for potential dual-cache loading
                    self._cache_base_dir = self.cache_dir.parent
        ...
        # Additional state for dual-cache support
        self._cache_mapping_split: Optional[np.ndarray] = None  # 'train' or 'val' per sample
        
        if cache_mapping is not None and Path(cache_mapping).exists():
            try:
                self._cache_mapping_path = Path(cache_mapping)  # Store for reload
                mapping_data = np.load(cache_mapping, allow_pickle=True, mmap_mode='r')
                ...
                # Check for dual-cache mapping (has cache_split field)
                if 'cache_split' in mapping_data.files:
                    self._cache_mapping_split = mapping_data['cache_split']
                    # Count samples per cache
                    train_count = np.sum(self._cache_mapping_split == 'train')
                    val_count = np.sum(self._cache_mapping_split == 'val')
                    print(f"[CACHE] Dual-cache mapping: {train_count:,} from train, {val_count:,} from val")
                    
                    # Load the alternate cache reader if needed
                    if val_count > 0 and self._consolidated_reader is not None and self._cache_base_dir is not None:
                        alt_cache_name = 'val' if 'train' in str(self.cache_dir) else 'train'
                        alt_cache_dir = self._cache_base_dir / alt_cache_name
                        if (alt_cache_dir / "manifest.json").exists():
                            try:
                                self._consolidated_reader_alt = ConsolidatedCacheReader(alt_cache_dir)
                                print(f"[CACHE] Loaded alternate cache ({alt_cache_name}): {len(self._consolidated_reader_alt):,} samples")
                            except Exception as e:
                                print(f"[CACHE] Warning: Failed to load alternate cache: {e}")
    ...
    def _load_labels(self) -> List[Dict[str, Any]]:
        ...
        if npy_files_path.exists() and npy_labels_path.exists():
            ...
            if skip_files_array:
                ...
                # Try memory-mapped mode first, fall back to regular load for object dtypes
                try:
                    files = np.load(npy_files_path, mmap_mode='r')
                except ValueError as e:
                    if "can't be memory-mapped" in str(e) or "Python objects" in str(e):
                        print(f"[LABELS] Files array has object dtype, loading without mmap...")
                        files = np.load(npy_files_path, allow_pickle=True)
                    else:
                        raise
    ...
    def __getitem__(self, idx: int):
        ...
        if getattr(self, '_use_numpy', False):
            ...
            if self._numpy_files is not None:
                ...
                if getattr(self, '_files_are_bytes', False):
                    ...
                    # Object dtype array with bytes objects
                    file_path = file_bytes.decode('utf-8')
                else:
        ...
        if self._use_cache_mapping and self._consolidated_reader is not None:
            # Initialize fallback indices cache once (thread-safe via GIL)
            # Use getattr with sentinel to handle both missing and None cases
            if getattr(self, '_fallback_count', -1) < 0:
                self._fallback_valid_indices = np.where(self._cache_mapping_valid)[0]
                self._fallback_count = len(self._fallback_valid_indices)
                invalid_count = len(self._cache_mapping_valid) - self._fallback_count
                if invalid_count > 0:
                    pct = invalid_count / len(self._cache_mapping_valid) * 100
                    print(f"[CACHE] Warning: {invalid_count:,} samples ({pct:.1f}%) missing from cache, will substitute valid samples", flush=True)
            ...
            if self._cache_mapping_valid[idx]:
                try:
                    ...
                    # Select correct cache reader based on cache_split (dual-cache support)
                    reader = self._consolidated_reader
                    if self._cache_mapping_split is not None and self._consolidated_reader_alt is not None:
                        sample_split = str(self._cache_mapping_split[idx])
                        # If this sample is from the alternate cache (e.g., val cache when training)
                        current_cache_is_train = 'train' in str(self.cache_dir)
                        if (current_cache_is_train and sample_split == 'val') or \
                           (not current_cache_is_train and sample_split == 'train'):
                            reader = self._consolidated_reader_alt
                    
                    features = reader._read_sample(shard_id, offset)
                    return make_result(features)
                except Exception as e:
                    # Shard read failed - use fallback
                    if self._cache_debug:
                        print(f"[CACHE] Shard read failed for idx {idx}: {e}", flush=True)
            
            # Invalid cache mapping OR shard read failed - use deterministic fallback
            if self._fallback_count > 0:
                # Pick a deterministic fallback based on idx to maintain reproducibility
                # Try up to 3 different fallback indices in case some fail
                for attempt in range(3):
                    fallback_pos = (idx + attempt * 12345) % self._fallback_count
                    fallback_idx = int(self._fallback_valid_indices[fallback_pos])
                    
                    try:
                        shard_id = int(self._cache_mapping_shards[fallback_idx])
                        offset = int(self._cache_mapping_offsets[fallback_idx])
                        
                        # Select correct cache reader for fallback too
                        reader = self._consolidated_reader
                        if self._cache_mapping_split is not None and self._consolidated_reader_alt is not None:
                            sample_split = str(self._cache_mapping_split[fallback_idx])
                            current_cache_is_train = 'train' in str(self.cache_dir)
                            if (current_cache_is_train and sample_split == 'val') or \
                               (not current_cache_is_train and sample_split == 'train'):
                                reader = self._consolidated_reader_alt
                        ...
                        features = reader._read_sample(shard_id, offset)
                        # Use the ORIGINAL label (idx), not fallback's label
                        return make_result(features)
                    except Exception as e:
                        if self._cache_debug:
                            print(f"[CACHE] Fallback attempt {attempt+1} failed for idx {fallback_idx}: {e}", flush=True)
                        continue
                # All fallback attempts failed - fall through to path-based lookup
        ...
        if features is None:
            ...
            if audio_path is None or not audio_path.exists():
                # For samples missing from cache, use the pre-computed fallback mechanism
                if self._use_cache_mapping and hasattr(self, '_fallback_count') and self._fallback_count > 0:
                    # Try up to 3 different fallback indices
                    for attempt in range(3):
                        fallback_pos = (idx + attempt * 54321) % self._fallback_count
                        fallback_idx = int(self._fallback_valid_indices[fallback_pos])
                        try:
                            shard_id = int(self._cache_mapping_shards[fallback_idx])
                            offset = int(self._cache_mapping_offsets[fallback_idx])
                ...
                cache_mapping_status = "valid" if (self._use_cache_mapping and self._cache_mapping_valid is not None and self._cache_mapping_valid[idx]) else "invalid/missing"
                raise RuntimeError(
        ...
        if npy_files_path.exists() and npy_labels_path.exists():
            ...
            if skip_files_array:
                ...
                try:
                    ...
                    if "can't be memory-mapped" in str(e) or "Python objects" in str(e):
                        self._numpy_files = np.load(npy_files_path, allow_pickle=True)
                    else:
                        raise
    ...
    def _ensure_consolidated_cache_loaded(self) -> None:
        ...
        Note: verbose=False to avoid corrupting tqdm progress bar in Git Bash.
        """
            ...
            if consolidated_manifest.exists():
                try:
                    self._consolidated_reader = ConsolidatedCacheReader(
                        self.cache_dir, skip_index=skip_index, verbose=False
                    )
                ...
                if (consolidated_alt / "manifest.json").exists():
                    try:
                        self._consolidated_reader = ConsolidatedCacheReader(
                            consolidated_alt, skip_index=skip_index, verbose=False
                        )
                    except (OSError, ValueError, KeyError, json.JSONDecodeError) as e:
...
def _atomic_torch_save(obj: Any, path: Path) -> None:
    ...
    try:
        # Save to temp file with explicit flush
...
def train_epoch(
    ...
    *,
    ...
    progress_initial_batches: int = 0,
    progress_total_batches_override: Optional[int] = None,
    ...
    Args:
        ...
        should_stop: Optional callback returning True when training should stop ASAP (used for Ctrl+C graceful shutdown)
        progress_callback: Optional callback(batch_index, total_batches) to report latest processed batch
    ...
    display_total_batches = int(progress_total_batches_override) if progress_total_batches_override else total_batches
    ...
    def safe_cuda_step(step_fn, description: str = "optimizer step") -> bool:
        """Execute a CUDA operation with error recovery.

        Returns:
            True if the operation succeeded, False if it was skipped after retries.
        """
    ...
    # Force per-iteration refresh so Git Bash shows live metrics on the same bar line.
    pbar = tqdm(
        dataloader,
        desc="Training",
        file=sys.stdout,
        total=display_total_batches,
        initial=int(progress_initial_batches),
        dynamic_ncols=True,
        miniters=1,
        mininterval=0.0,
        smoothing=0.0,
        # Explicitly include postfix to ensure loss/acc are visible in Git Bash.
        bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}] {postfix}",
    )
    # Manual iteration lets us check for shutdown BEFORE fetching the next batch.
    # This prevents Windows DataLoader pin_memory thread crashes when Ctrl+C is pressed mid-epoch.
    it = iter(pbar)
    batch_index = 0
    while True:
        ...
        try:
            ...
            # If user requested shutdown, treat DataLoader failures during teardown as a clean interrupt.
            if should_stop is not None and should_stop():
                raise KeyboardInterrupt
            raise

        batch_index += 1
        epoch_batch_index = int(progress_initial_batches) + batch_index
        if progress_callback is not None:
            progress_callback(epoch_batch_index, display_total_batches)
        # Periodic CUDA memory cleanup to prevent fragmentation
        if device.type == "cuda" and epoch_batch_index % cuda_cleanup_interval == 0:
            torch.cuda.empty_cache()
        ...
        if amp_enabled and scaler is not None:
            ...
            should_step = epoch_batch_index % accum_steps == 0 or batch_index == total_batches
            if should_step:
                if use_sam:
                    ...
                    step_ok = safe_cuda_step(amp_optimizer_step, "AMP optimizer step")
                    if not step_ok:
                        # CUDA error recovery - skip to next batch
                        continue
                
                # AWP (Adversarial Weight Perturbation) step for AMP path
                if awp is not None and awp.should_attack(current_epoch, epoch_batch_index, awp_freq):
                    if use_mixup:
            ...
            if should_step:
                ...
                if use_sam:
                    ...
                    step_ok = safe_cuda_step(non_amp_optimizer_step, "optimizer step")
                    if not step_ok:
                        # CUDA error recovery - skip to next batch
                        continue
        ...
        # Update bar postfix with realtime metrics (same row as the progress bar).
        cur_loss = loss.item()
        cur_acc = 100 * correct / max(total, 1)
        pbar.set_postfix(loss=f"{cur_loss:.4f}", acc=f"{cur_acc:.2f}%", refresh=True)

        # Mid-epoch checkpoint (protects against crashes during long epochs)
        if checkpoint_callback and checkpoint_every_batches > 0:
            if epoch_batch_index % checkpoint_every_batches == 0 and epoch_batch_index < display_total_batches:
                checkpoint_callback(epoch_batch_index, display_total_batches)
    ...
    *,
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(pbar, start=1):
            ...
            # Realtime validation metrics on the progress bar.
            cur_mean_loss = total_loss / max(batch_index, 1)
            cur_acc = 100 * correct / max(total, 1)
            pbar.set_postfix(loss=f"{cur_mean_loss:.4f}", acc=f"{cur_acc:.2f}%", refresh=True)
...
def validate_with_tta(
    ...
    *,
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validation (TTA)", ncols=120, dynamic_ncols=False):
            # Handle both (features, labels) and (features, labels, velocities) formats
...
def main():
    ...
    parser.add_argument(
        "--reset-scheduler",
        action="store_true",
        help="Reset LR scheduler when resuming (don't load scheduler state from checkpoint). Use when changing phases or LR settings.",
    )
    parser.add_argument(
        "--min-lr",
        type=float,
    ...
    if args.cache_mapping:
        ...
        # Try both naming conventions: "train_cache_mapping.npz" and "cache_mapping.npz"
        for train_name in ["train_cache_mapping.npz", "cache_mapping.npz"]:
            train_mapping = labels_dir / train_name
            if train_mapping.exists():
                train_cache_mapping = train_mapping
                print(f"[CACHE] Auto-detected train cache mapping: {train_mapping}")
                break
        val_labels_dir = resolve_labels("val", val_labels_file).parent
        for val_name in ["val_cache_mapping.npz", "cache_mapping.npz"]:
            val_mapping = val_labels_dir / val_name
            if val_mapping.exists():
                val_cache_mapping = val_mapping
    ...
    balanced_sample_weights_tensor: Optional[torch.Tensor] = None
    balanced_epoch_samples: Optional[int] = None
    use_deterministic_balanced_sampler = False
    if args.balanced_sampling:
        ...
        if use_torchsampler:
            ...
            # Use our deterministic weighted sampler so we can resume mid-epoch without replaying earlier batches.
            # This produces the same distribution as WeightedRandomSampler (i.i.d. w/ replacement), but with a
            # fixed per-epoch seed.
            use_deterministic_balanced_sampler = True
            balanced_sample_weights_tensor = sample_weights_tensor
            balanced_epoch_samples = int(epoch_samples)
            balanced_sampler = None
    ...
    # Build initial train_loader. For deterministic balanced sampling we use epoch-seeded sampling.
    if args.balanced_sampling and use_deterministic_balanced_sampler and balanced_sample_weights_tensor is not None and balanced_epoch_samples is not None:
        base_seed = int(args.seed) if args.seed is not None else 0
        epoch_seed = base_seed + 0
        det_sampler0 = _DeterministicWeightedSampler(
            weights=balanced_sample_weights_tensor,
            num_samples=balanced_epoch_samples,
            seed=epoch_seed,
            start_sample_index=0,
        )
        dl_gen0 = torch.Generator(device="cpu")
        dl_gen0.manual_seed((epoch_seed + 10_000_000) % (2**63 - 1))
    ...
    if checkpoint_interval:
        print(f"Epoch checkpoints: every {checkpoint_interval} epochs")
    if checkpoint_batch_interval:
        print(f"Mid-epoch checkpoints: every {checkpoint_batch_interval} batches")
    else:
        print("Mid-epoch checkpoints: DISABLED")
    ...
    # Mid-epoch resume tracking (populated only when resuming from a mid-epoch checkpoint)
    resume_mid_epoch_batch_index: int = 0  # batches already completed in the epoch
    resume_mid_epoch_total_batches: int = 0
    
    # Track ACTUAL training progress across all resumes
    # This prevents epoch inflation from repeated mid-epoch restarts
    cumulative_batches_trained: int = 0  # Total batches actually trained (persisted)
    batches_per_epoch: int = 0  # Will be set after dataloader is created

    # Track progress within the current epoch so Ctrl+C can save a mid-epoch checkpoint.
    last_train_batch_index: int = 0
    last_train_total_batches: int = 0

    # Optional human-readable progress logging (throttled to avoid spam).
    # This complements tqdm (useful when logs are captured or tqdm rendering is imperfect).
    last_progress_log_time: float = 0.0
    last_progress_log_batch: int = 0
    progress_log_every_seconds: float = 60.0
    progress_log_every_batches: int = int(checkpoint_batch_interval or 5000)

    def _record_train_progress(batch_idx: int, total_batches: int) -> None:
        nonlocal last_train_batch_index, last_train_total_batches
        nonlocal last_progress_log_time, last_progress_log_batch
        last_train_batch_index = int(batch_idx)
        last_train_total_batches = int(total_batches)
        # Progress is shown by tqdm bar - no need for extra logging
    ...
    def save_checkpoint(
        ...
        *,
        ...
        checkpoint_payload = {
            ...
            # RNG state (enables deterministic resume when args.seed is set)
            "rng": {
        ...
        if is_mid_epoch:
            ...
            # Use tqdm.write to avoid interference with progress bar
            from tqdm import tqdm
            tqdm.write(f"\n[CHECKPOINT] Mid-epoch checkpoint SAVED (epoch {epoch_index}, batch {batch_index:,}/{total_batches:,}, {pct:.0f}%)")
            tqdm.write(f"   -> {checkpoint_path.name} + latest_checkpoint.pth")
    ...
    if args.resume_from:
        ...
        if not has_mismatch:
            try:
                ...
                _safe_print(f"    [WARN] Could not load optimizer state (optimizer type changed?): {e}")
                _safe_print("    ℹ️  Starting with fresh optimizer state - this is fine for phase transitions.")
                # Clear any partial state that may have been loaded
                optimizer.state.clear()
                for group in optimizer.param_groups:
                    for p in group['params']:
                        if p in optimizer.state:
                            del optimizer.state[p]
                _safe_print("    ✓ Optimizer state cleared successfully.\n")
            scheduler_state = checkpoint_state.get("scheduler_state")
            if getattr(args, 'reset_scheduler', False):
                _safe_print("    ℹ️  --reset-scheduler: Using fresh scheduler (ignoring checkpoint scheduler state).")
                # Also reset initial_lr in optimizer param_groups to match new --lr
                # Otherwise scheduler uses old initial_lr from checkpoint
                for group in optimizer.param_groups:
                    group["initial_lr"] = args.lr
                    group["lr"] = args.lr
                _safe_print(f"    ✓ Reset optimizer LR to {args.lr} (was {checkpoint_state.get('optimizer_state', {}).get('param_groups', [{}])[0].get('initial_lr', 'unknown')})\n")
            elif scheduler_state is not None:
                try:
                    scheduler.load_state_dict(scheduler_state)
                except Exception as e:
                    _safe_print(f"    [WARN] Could not load scheduler state: {e}")
        ...
        if has_mismatch:
            ...
            ckpt_epoch = int(checkpoint_state.get("epoch", 0))
            ckpt_batch_index = checkpoint_state.get("batch_index")
            ckpt_total_batches = checkpoint_state.get("total_batches")

            # Restore RNG state if present (best-effort).
            rng = checkpoint_state.get("rng")
            if isinstance(rng, dict):
                try:
                    ...
                    _safe_print(f"[WARN] Failed to restore RNG state from checkpoint: {e}")

            # Mid-epoch resume: checkpoints store the in-progress epoch (1-based) plus batch_index.
            if ckpt_batch_index is not None and ckpt_total_batches is not None:
                ckpt_args = checkpoint_state.get("args")
                ckpt_batch_size = None
                if isinstance(ckpt_args, dict):
                    ckpt_batch_size = ckpt_args.get("batch_size")

                if ckpt_batch_size is not None and int(ckpt_batch_size) != int(args.batch_size):
                    _safe_print(
                        "[WARN] Mid-epoch resume requested but batch_size differs from checkpoint; "
                        "resuming at start of epoch instead."
                    )
                    start_epoch = ckpt_epoch
                    last_completed_epoch = start_epoch
                else:
                    resume_mid_epoch_batch_index = int(ckpt_batch_index)
                    resume_mid_epoch_total_batches = int(ckpt_total_batches)
                    start_epoch = max(0, ckpt_epoch - 1)
                    last_completed_epoch = start_epoch
                    print(
                        f"Resuming from MID-EPOCH checkpoint {args.resume_from} "
                        f"(epoch {ckpt_epoch}, batch {resume_mid_epoch_batch_index:,}/{resume_mid_epoch_total_batches:,})"
                    )
            else:
                start_epoch = ckpt_epoch
                last_completed_epoch = start_epoch
            ...
            ckpt_cumulative_batches = checkpoint_state.get("cumulative_batches_trained", 0)
            ckpt_batches_per_epoch = checkpoint_state.get("batches_per_epoch", 0)
            if ckpt_cumulative_batches > 0:
                ...
                if ckpt_batches_per_epoch > 0:
                    ...
                    print(f"[PROGRESS] Cumulative batches trained: {cumulative_batches_trained:,} "
                          f"(~{actual_epochs:.1f} full epochs)")
                    if ckpt_epoch > 0 and actual_epochs < ckpt_epoch * 0.8:
                        print(f"[INFO] Note: Epoch counter ({ckpt_epoch}) > actual training ({actual_epochs:.1f} epochs)")
                        print(f"[INFO] This is normal if you've resumed mid-epoch multiple times.")
            else:
                # Legacy checkpoint without cumulative tracking - estimate from epoch
                if ckpt_batch_index is not None and ckpt_total_batches is not None:
                    # Mid-epoch: estimate as (completed_epochs * batches) + current_batch
                    cumulative_batches_trained = (start_epoch * ckpt_total_batches) + ckpt_batch_index
                else:
                    # End of epoch: estimate as epoch * estimated_batches
                    cumulative_batches_trained = start_epoch * 118891  # Will be corrected once dataloader created
                print(f"[INFO] Legacy checkpoint - estimating cumulative progress: ~{cumulative_batches_trained:,} batches")
    ...
    # Set batches_per_epoch now that train_loader is created
    batches_per_epoch = len(train_loader)
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            print(f"\nEpoch {epoch + 1}/{args.epochs} (actual training: ~{actual_full_epochs:.1f} full epochs)")
            print("-" * 60)

            train_progress_initial_batches = 0
            train_progress_total_override: Optional[int] = None

            # === TRUE MID-EPOCH RESUME (deterministic balanced sampling path) ===
            # Rebuild the train_loader each epoch so sampling is deterministic per epoch, and so we can
            # resume mid-epoch by starting the sampler at an offset.
            if (
                args.balanced_sampling
                and use_deterministic_balanced_sampler
                and balanced_sample_weights_tensor is not None
                and balanced_epoch_samples is not None
                and not use_shard_aware
                and curriculum_scheduler is None
            ):
                base_seed = int(args.seed) if args.seed is not None else 0
                epoch_seed = base_seed + int(epoch)

                start_sample_index = 0
                if resume_mid_epoch_batch_index > 0 and epoch == start_epoch:
                    start_sample_index = resume_mid_epoch_batch_index * int(args.batch_size)
                    start_sample_index = min(start_sample_index, balanced_epoch_samples)
                    train_progress_initial_batches = int(resume_mid_epoch_batch_index)
                    train_progress_total_override = int(resume_mid_epoch_total_batches) if resume_mid_epoch_total_batches else None

                det_sampler = _DeterministicWeightedSampler(
                    weights=balanced_sample_weights_tensor,
                    num_samples=balanced_epoch_samples,
                    seed=epoch_seed,
                ...
                dl_gen.manual_seed((epoch_seed + 10_000_000) % (2**63 - 1))
                train_loader = build_loader(
                    train_dataset,
                    batch_size=args.batch_size,
                    shuffle=False,
                ...
                if start_sample_index > 0:
                    _safe_print(
                        f"[RESUME] Continuing epoch {epoch + 1} from batch {resume_mid_epoch_batch_index + 1:,}/"
                        f"{resume_mid_epoch_total_batches:,} (skipped {resume_mid_epoch_batch_index:,} batches)"
                    )
                    # Only apply the mid-epoch offset once.
                    resume_mid_epoch_batch_index = 0
                    resume_mid_epoch_total_batches = 0
            ...
            train_loss, train_acc = train_epoch(
                ...
                progress_callback=_record_train_progress,
                progress_initial_batches=train_progress_initial_batches,
                progress_total_batches_override=train_progress_total_override,
            ...
            if epoch == start_epoch and resume_mid_epoch_batch_index > 0:
                batches_this_epoch = batches_per_epoch - resume_mid_epoch_batch_index
            else:
                batches_this_epoch = batches_per_epoch
            ...
            # Free validation tensors before logging/checkpointing to prevent fragmentation
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            ...
            if args.scheduler == "plateau":
                ...
                # Always step the scheduler to keep it aligned with training epochs
                # During warmup, LR is manually overridden above, so scheduler LR is ignored
                # This ensures CosineAnnealingWarmRestarts T_0 aligns with actual epoch count
                scheduler.step()

            # Log current learning rate and actual training progress
            current_lr = scheduler.get_last_lr()[0] if hasattr(scheduler, 'get_last_lr') else args.lr
            actual_epochs_completed = cumulative_batches_trained / max(batches_per_epoch, 1)
            print(f"Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.2f}%")
            if val_balanced_acc is not None:
                print(f"Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.2f}% | Val Balanced Acc: {val_balanced_acc:.2f}%")
            else:
                print(f"Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.2f}%")
            print(f"Learning Rate: {current_lr:.2e} | Actual Training: {actual_epochs_completed:.1f} full epochs")
            
            # Overfitting diagnostic
            train_val_gap = train_acc - (val_balanced_acc if val_balanced_acc is not None else val_acc)
            if train_val_gap > 15:
                print(f"\n[DIAGNOSTIC] Overfitting detected:")
                print(f"   Train/Val gap: {train_val_gap:.1f}% (train {train_acc:.1f}% vs val {val_balanced_acc or val_acc:.1f}%)")
                print(f"   This is NORMAL for imbalanced datasets with uniform sampling.")
                print(f"   The model learns rare classes well (high train acc) but struggles")
                print(f"   to generalize because those classes have limited examples.")
                print(f"   Best checkpoint is preserved - training continues to find better solutions.")

            # CLASS HEALTH CHECK: Run every epoch to monitor for class collapse
            # This provides visibility into per-class accuracy throughout training
            ...
            if has_collapsed_classes and val_metric_for_best > best_val_acc:
                _safe_print(f"⚠️  Skipping best checkpoint save: {num_collapsed_classes} classes at 0% accuracy")
                _safe_print(f"    (balanced_acc={val_metric_for_best:.2f}% is misleading with collapsed classes)")
        ...
        # Ctrl+C mid-epoch on Windows can manifest as DataLoader pin-memory failures.
        is_pin_memory_teardown = isinstance(e, RuntimeError) and "Pin memory thread exited unexpectedly" in str(e)
        if isinstance(e, RuntimeError) and not _shutdown_requested and not is_pin_memory_teardown:
            raise

        print("\n[STOP] Training interrupted. Saving checkpoint...")
        # Prefer saving a mid-epoch checkpoint if we were inside an epoch.
        if last_train_batch_index > 0 and last_train_total_batches > 0:
            # Update cumulative batches with partial progress before saving
            # Account for batches trained in this interrupted epoch
            if epoch == start_epoch and resume_mid_epoch_batch_index > 0:
                # Resumed mid-epoch, trained from resume point to current
                batches_in_interrupted_epoch = last_train_batch_index - resume_mid_epoch_batch_index
            else:
                # Started fresh epoch, trained from 0 to current
                batches_in_interrupted_epoch = last_train_batch_index
            cumulative_batches_trained += max(0, batches_in_interrupted_epoch)
            
            actual_epochs = cumulative_batches_trained / max(batches_per_epoch, 1)
            print(f"[PROGRESS] Total batches trained: {cumulative_batches_trained:,} (~{actual_epochs:.1f} full epochs)")
            
            # 'epoch' is still in-scope here (loop variable). Save progress within the current epoch.
            save_checkpoint(
                epoch + 1,
                reason="keyboard_interrupt_mid_epoch",
                batch_index=last_train_batch_index,
                total_batches=last_train_total_batches,
            )
        else:
