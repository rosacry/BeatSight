...
class InstrumentPitchRanker:
    ...
    def _extract_features(
        ...
        for event in events:
            ...
            try:
                ...
                logger.debug(
                    "Spectral centroid extraction failed at %.3fs: %s",
                    event.timestamp,
                    e,
                )
    ...
    def _cluster_and_rank(
        ...
        mean_centroids = {cid: np.mean(vals) for cid, vals in cluster_centroids.items()}

        # Rank clusters by centroid (high to low by default)
        if config.ranking_strategy == RankingStrategy.PITCH_HIGH_TO_LOW:
            sorted_clusters = sorted(
                mean_centroids.keys(), key=lambda c: mean_centroids[c], reverse=True
            )
        elif config.ranking_strategy == RankingStrategy.PITCH_LOW_TO_HIGH:
            sorted_clusters = sorted(
                mean_centroids.keys(), key=lambda c: mean_centroids[c]
            )
            ...
            sorted_clusters = sorted(
                first_occurrence.keys(), key=lambda c: first_occurrence[c]
            )
