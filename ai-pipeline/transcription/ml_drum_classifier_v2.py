...
class ConvBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_residual and identity is not None:
            ...
            out = out + self.residual_scale * F.interpolate(
...
class DrumClassifierCNNv2(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = ConvBlock(
            1, c1, use_se=use_se, se_reduction=se_reduction, use_residual=False
        )
        self.conv2 = ConvBlock(
            c1, c2, use_se=use_se, se_reduction=se_reduction, use_residual=use_residual
        )
        self.conv3 = ConvBlock(
            c2, c3, use_se=use_se, se_reduction=se_reduction, use_residual=use_residual
        )
