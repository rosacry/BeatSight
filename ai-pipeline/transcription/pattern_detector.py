...
class CrashBuildDetector:
    ...
    def detect(self, events: List[DrumEvent]) -> List[DetectedPattern]:
        ...
        cymbal_events = [
            (i, e) for i, e in enumerate(events) if self._is_cymbal(e.label)
        ]
    ...
    def _is_cymbal(self, label: str) -> bool:
        ...
        return any(cym in label_lower for cym in self.config.crash_build_instruments)

    def _try_build_from(
        self, cymbal_events: List[Tuple[int, DrumEvent]], start_idx: int
...
class AccentTapDetector:
    ...
    def _is_valid_instrument(self, instrument: str) -> bool:
        ...
        for inst in self.config.accent_tap_instruments:
            ...
            if (
                inst_lower in ("hihat_closed", "hihat_open")
                and instrument_lower == "hihat"
            ):
                return True
    ...
    def _detect_in_instrument(
        self, inst_events: List[Tuple[int, DrumEvent]], instrument: str
    ...
    def _try_pattern_from(
        self, inst_events: List[Tuple[int, DrumEvent]], start_idx: int, instrument: str
    ) -> Optional[DetectedPattern]:
        ...
        confidence = (
            0.4 * alternation_score
            + 0.3 * timing_consistency
            + 0.3 * min(1.0, dynamic_range * 2)
        )
...
class HiHatBarkDetector:
    ...
    def _find_individual_barks(self, events: List[DrumEvent]) -> List[DetectedPattern]:
    ...
    def _find_continuous_barks(
        self, barks: List[DetectedPattern], events: List[DrumEvent]
    ) -> List[DetectedPattern]:
            ...
            if len(sequence) >= self.config.continuous_bark_min_count:
                ...
                if regularity >= self.config.continuous_bark_regularity:
                    ...
                    pattern = DetectedPattern(
                        ...
                        * np.mean([b.confidence for b in sequence]),
                        ...
                        properties={
                            ...
                            "barks_per_second": len(sequence)
                            / (sequence[-1].end_time - sequence[0].start_time + 0.001),
...
class HiHatSplashDetector:
    ...
    def _analyze_sustain(self, events: List[DrumEvent], idx: int) -> Dict[str, Any]:
...
class CrescendoDecrescendoDetector:
    ...
    def _check_dynamic_change(
        self, window: List[DrumEvent], is_crescendo: bool
    ) -> Optional[DetectedPattern]:
        ...
        if (
            not is_crescendo
            and slope_per_second > -self.config.dynamic_change_min_slope
        ):
            return None
        ...
        pattern_type = (
            PatternType.CRESCENDO if is_crescendo else PatternType.DECRESCENDO
        )
            ...
            for next_p in type_patterns[1:]:
                ...
                if next_p.start_time <= current.end_time:
                    ...
                    current = DetectedPattern(
                        ...
                        event_indices=list(
                            set(current.event_indices + next_p.event_indices)
                        ),
...
class PatternDetector:
    ...
    def detect(self, events: List[DrumEvent]) -> List[DetectedPattern]:
        ...
        filtered = [
            p for p in all_patterns if p.confidence >= self.config.min_confidence
        ]

        logger.info(
            f"Detected {len(filtered)} patterns (filtered from {len(all_patterns)})"
        )

        return filtered

    def annotate_events(
        self, events: List[DrumEvent], patterns: List[DetectedPattern]
    ) -> List[DrumEvent]:
        """
            ...
            for idx in pattern.event_indices:
                if 0 <= idx < len(events):
                    ...
                    if pattern.pattern_type == PatternType.HIHAT_BARK:
                        ...
                        if (
                            events[idx].velocity
                            >= self.config.accent_tap_velocity_threshold
                        ):
...
def detect_all_patterns(
    events: List[DrumEvent], config: Optional[PatternDetectorConfig] = None
) -> List[DetectedPattern]:
    """
...
def annotate_transcription_result(
    ...
    for i, event in enumerate(annotated):
        original = (
            events[event.original_index] if event.original_index is not None else {}
        )
        annotated_dict = {
            **original,
