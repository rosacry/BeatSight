...
from app.services.contribution_quality import (
    ContributionQualityService,
    ConflictType,
)
...
async def submit_contribution(
    ...
    # ==========================================================================
    # Quality Gate: Comprehensive conflict and quality checks
    # ==========================================================================
    quality_service = ContributionQualityService(db)
    
    # Run comprehensive conflict validation
    quality_result = await quality_service.check_contribution_conflicts(
        map_version_id=request.map_version_id,
        onset_time_ms=request.onset_time_ms,
        user_id=current_user.id,
    )
    
    if quality_result.has_conflict and quality_result.blocking:
        # Handle different blocking conflict types with appropriate responses
        if quality_result.conflict_type == ConflictType.PENDING_EDIT:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"You have a pending edit proposal on this region. "
                f"Wait for it to be reviewed before submitting another correction. "
                f"Details: {quality_result.message}",
            )
        elif quality_result.conflict_type == ConflictType.STALE_MODEL:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"This beatmap was generated by an older model version. "
                f"A newer model may produce different results. "
                f"Details: {quality_result.message}",
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=quality_result.message or "Quality validation failed",
            )

    # Note if there are non-blocking conflicts (for weight calculation)
    has_conflicts = quality_result.has_conflict
    ...
    requires_review = (
        ...
        or has_conflicts  # Has conflicts from quality check
    )
    ...
    # Calculate initial training weight based on user karma
    # Full weight calculation happens at export time with calculate_contribution_weight()
    base_karma = current_user.karma_score or 0
    # Simple karma scaling: 100 karma = 1.0, 1000+ karma = 1.3
    if base_karma >= 1000:
        training_weight = 1.3
    elif base_karma >= 500:
        training_weight = 1.15
    elif base_karma >= 100:
        training_weight = 1.0
    else:
        training_weight = 0.9
    ...
    if has_conflicts:
        training_weight *= 0.7  # 30% penalty for conflicting contributions
    ...
    contribution = TrainingContribution(
        ...
        consensus_count=1,  # User agrees with themselves
    )
    ...
    # This ensures existing contributions get updated conflict/consensus flags
    await quality_service.update_consensus_counts(
        map_version_id=request.map_version_id,
        onset_time_ms=request.onset_time_ms,
        new_corrected_component=request.corrected_component,
    )
    await db.commit()
