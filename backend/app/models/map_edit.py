...
class MapEditProposal(Base):
    ...
    status: Mapped[EditStatus] = mapped_column(
        SAEnum(EditStatus), default=EditStatus.PENDING, nullable=False
    )
    ...
    map_version: Mapped["MapVersion"] = relationship(
        "MapVersion", back_populates="edit_proposals"
    )
...
class MapVerificationDecision(Base):
    ...
    proposal_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("map_edit_proposals.id", ondelete="CASCADE"),
        unique=True,
    )
    ...
    decision: Mapped[VerificationDecision] = mapped_column(
        SAEnum(VerificationDecision), nullable=False
    )
    ...
    proposal: Mapped[MapEditProposal] = relationship(
        "MapEditProposal", back_populates="decision"
    )
    verifier: Mapped["User"] = relationship(
        "User", back_populates="verification_decisions"
    )
