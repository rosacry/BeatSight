...
class MapVersion(Base):
    ...
    map: Mapped["Map"] = relationship(
        "Map", back_populates="versions", foreign_keys=[map_id]
    )
    generation_job: Mapped["AIJob | None"] = relationship(
        "AIJob", back_populates="output_versions"
    )
    assets: Mapped[list["MapAsset"]] = relationship(
        "MapAsset", back_populates="map_version", cascade="all, delete-orphan"
    )
