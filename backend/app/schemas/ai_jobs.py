...
class AIGenerationOptions(BaseModel):
    ...
    quantization_grid: QuantizationGrid = Field(
        default=QuantizationGrid.SIXTEENTH,
...
class AIJobEnqueueResponse(BaseModel):
    ...
    job: AIJobRead
    queue_position: int | None = Field(description="Position in queue (0-based), null if already processing")
    estimated_wait_minutes: int | None = Field(description="Estimated wait time in minutes")
