...
from app.models import Achievement, UserAchievement, AIJob, MapEditProposal, User
...
async def check_beatmap_generation_achievements(
    ...
    if count >= 25:
        if await check_and_award_achievement(db, user_id, ACHIEVEMENT_TWENTY_FIVE_BEATMAPS):
...
async def check_edit_achievements(
    ...
    result = await db.execute(
        ...
        .where(MapEditProposal.proposer_id == user_id)
    )
    count = result.scalar() or 0
