...
class AcoustIDService:
    ...
    def __init__(
        ...
        """
        self.api_key = (
            api_key or os.getenv("ACOUSTID_API_KEY") or os.getenv("ACOUSTID_KEY")
        )
    ...
    async def identify_audio(
        ...
        try:
            ...
            logger.error(
                "AcoustID identification failed", error=str(e), path=audio_path
            )
            return None
    ...
    async def acquire(self) -> None:
        ...
        async with self._lock:
            ...
            if len(self._requests) >= self.max_requests:
                ...
                wait_time = (
                    oldest + timedelta(seconds=self.window) - now
                ).total_seconds()
                if wait_time > 0:
