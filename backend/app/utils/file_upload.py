...
MAGIC_BYTES = {
    ...
    # Note: WebP also uses RIFF but WAV has priority; WebP detection needs content check
    # Documents
    ...
    def validate(
        ...
        if self.allowed_types and detected_type:
            if detected_type not in self.allowed_types:
                ...
                errors.append(
                    f"File type '{detected_type}' not allowed. Allowed: {allowed}"
                )
...
class ProgressTracker:
    ...
    def cleanup_completed(self) -> int:
        ...
        """
        to_remove = [
            uid for uid, progress in self._uploads.items() if progress.is_complete
        ]
    ...
    def list_active(self) -> list[UploadProgress]:
        ...
        """
        return [
            progress for progress in self._uploads.values() if not progress.is_complete
        ]
