                    ...
                    if task.scheduled_at and task.scheduled_at > datetime.now(timezone.utc):
                        ...
                        if timeout is not None:
                            ...
                            if remaining <= 0:
                                return None
                            wait_time = min(
                                (task.scheduled_at - datetime.now(timezone.utc)).total_seconds(),
                                remaining,
                            )
                        else:
                            return None
...
class Worker:
    ...
    def __init__(
        ...
        num_workers: int = 1,
        name: str = "worker",
        on_task_complete: Optional[Callable[[Task, TaskResult], Awaitable[None]]] = None,
            ...
            try:
                ...
                if result.success:
                    ...
                    if task.can_retry:
                        ...
                        await self._on_error(task, Exception(result.error or "Unknown error"))
...
class Scheduler:
    ...
    def schedule(
        ...
        job = ScheduledJob(
            ...
            args=args,
            kwargs=kwargs or {},
            next_run=datetime.now(timezone.utc) if run_immediately
