        ...
        *,
        ...
        if trans_def and trans_def.guard:
            if not trans_def.check_guard(context):
                raise GuardError(
                    f"Guard failed for transition {self._state} -> {to_state}"
                )
        ...
        for (from_state, to_state), trans_def in self._transition_defs.items():
            if from_state == self._state and trans_def.event == event:
                if trans_def.guard is None or trans_def.check_guard(context):
                    self.transition(
                        to_state, event=event, metadata=metadata, context=context
                    )
                    return True
        return False
    ...
    async def transition(
        ...
        *,
        ...
        async with self._lock:
            ...
            if trans_def and trans_def.guard:
                guard_result = (
                    trans_def.guard(context) if context else trans_def.guard()
                )
            ...
            if trans_def and trans_def.before:
                await self._run_callback(
                    trans_def.before, context
                ) if context else await self._run_callback(trans_def.before)
            ...
            if trans_def and trans_def.after:
                await self._run_callback(
                    trans_def.after, context
                ) if context else await self._run_callback(trans_def.after)
    ...
    def execute(
        ...
        *,
        ...
        while current and iterations < max_iterations:
            if current in visited and current not in [
                s.on_failure for s in self._steps.values()
            ]:
