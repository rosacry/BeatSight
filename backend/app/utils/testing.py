...
def fake_future_datetime(
    *,
    ...
    return fake_datetime(
        start=now, end=now + timedelta(days=max_days), tz_aware=tz_aware
    )
...
def fake_past_datetime(
    *,
    ...
    now = datetime.now(timezone.utc) if tz_aware else datetime.now()
    return fake_datetime(
        start=now - timedelta(days=max_days), end=now, tz_aware=tz_aware
    )
...
def assert_almost_equal(
    ...
    if round(abs(expected - actual), places) != 0:
        raise AssertionError(
            f"Values not equal to {places} places: actual={actual}, expected={expected}"
        )
...
class DataBuilder:
    ...
    def build_list(
        self, count: int, vary_field: str | None = None
    ) -> list[dict[str, Any]]:
        """
