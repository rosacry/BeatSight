...
class TestAsyncBatcher:
    ...
    async def test_flushes_on_size(self):
        ...
        async with AsyncBatcher(batch_size=3, timeout=10.0, processor=process) as batcher:
            for i in range(5):
                await batcher.add(i)
    ...
    async def test_flushes_on_close(self):
        ...
        async with AsyncBatcher(batch_size=10, timeout=10.0, processor=process) as batcher:
            await batcher.add(1)
            await batcher.add(2)
...
class TestTimeoutContext:
    ...
    async def test_raises_on_timeout(self):
        ...
        with pytest.raises(asyncio.TimeoutError):
            async with timeout_context(0.05) as ctx:
                await asyncio.sleep(1.0)
