...
class TestStripeService:
    ...
    def test_is_configured_false(self, mock_settings_unconfigured):
        ...
        with patch(
            ...
            return_value=mock_settings_unconfigured,
        ):
            service = StripeService()
    ...
    def test_get_price_id(self, stripe_service):
        ...
        assert (
            stripe_service._get_price_id(SubscriptionPlan.PRO_MONTHLY)
            == "price_monthly"
        )
        assert (
            stripe_service._get_price_id(SubscriptionPlan.PRO_YEARLY) == "price_yearly"
        )
    ...
    def test_get_plan_from_price(self, stripe_service):
        ...
        assert (
            stripe_service._get_plan_from_price("price_monthly")
            == SubscriptionPlan.PRO_MONTHLY
        )
        assert (
            stripe_service._get_plan_from_price("price_yearly")
    ...
    async def test_create_checkout_not_configured(
        self, mock_settings_unconfigured, mock_session, mock_user
    ):
...
class TestWebhookHandlers:
    ...
    async def test_handle_subscription_deleted(self, stripe_service, mock_session):
        ...
        result = await stripe_service._handle_subscription_deleted(
            mock_session, subscription_data
        )
    ...
    async def test_sync_subscription_new(self, stripe_service, mock_session):
        ...
        result = await stripe_service._sync_subscription(
            mock_session, subscription_data
        )
