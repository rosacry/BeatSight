...
class TestFeatureFlag:
    ...
    def test_percentage_distribution(self):
        ...
        enabled_count = sum(1 for i in range(1000) if flag.is_enabled_for(f"user{i}"))
...
class TestFeatureFlags:
    ...
    def test_all_flags(self):
        ...
        flags = FeatureFlags(
            {
                "enabled": FeatureFlag(
                    name="enabled", status=FeatureFlagStatus.ENABLED
                ),
                "disabled": FeatureFlag(
                    name="disabled", status=FeatureFlagStatus.DISABLED
                ),
            }
        )
