...
class TestPermissionDefinitions:
    ...
    def test_role_permissions_are_valid(self):
        ...
        for role, perms in ROLE_PERMISSIONS.items():
            for perm in perms:
                assert isinstance(perm, Permission), f"Invalid permission {perm} in role {role}"
...
class TestRBACServiceUnit:
    ...
    @pytest.mark.asyncio
    async def test_get_user_permissions_combines_roles(self, rbac_service, mock_session):
    ...
    async def test_user_has_permission_true(self, rbac_service, mock_session):
        ...
        has_perm = await rbac_service.user_has_permission(user_id, Permission.ADMIN_DASHBOARD)
