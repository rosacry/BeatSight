...
class TestStorageConfigFromEnv:
    ...
    def test_azure_config_from_env(self) -> None:
        ...
        with patch.dict(
            ...
            assert config.azure_connection_string == "DefaultEndpointsProtocol=https;..."
...
class TestLocalStorageEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_stream_download_not_found(self, backend: LocalStorageBackend) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_metadata_without_sidecar(self, backend: LocalStorageBackend) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_with_metadata_sidecar(self, backend: LocalStorageBackend) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_nonexistent_is_noop(self, backend: LocalStorageBackend) -> None:
        """Test that deleting nonexistent file doesn't raise."""
        # Should not raise
        await backend.delete("definitely_not_there.txt")

    @pytest.mark.asyncio
    async def test_list_objects_empty_prefix(self, backend: LocalStorageBackend) -> None:
    ...
    @pytest.mark.asyncio
    async def test_list_objects_nonexistent_prefix(self, backend: LocalStorageBackend) -> None:
    ...
    @pytest.mark.asyncio
    async def test_path_traversal_double_dots(self, backend: LocalStorageBackend) -> None:
...
class TestS3StorageBackend:
    ...
    @pytest.mark.asyncio
    async def test_get_client_requires_aioboto3(self, backend: S3StorageBackend) -> None:
    ...
    async def test_stream_download(self, backend: S3StorageBackend) -> None:
        ...
        mock_body.iter_chunks = lambda chunk_size: AsyncIteratorMock([b"chunk1", b"chunk2"])
    ...
    @pytest.mark.asyncio
    async def test_get_presigned_url_put_with_content_type(self, backend: S3StorageBackend) -> None:
...
class TestAzureBlobStorageBackend:
    ...
    @pytest.mark.asyncio
    async def test_get_client_requires_azure_sdk(self, backend: AzureBlobStorageBackend) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_client_with_connection_string(self, backend: AzureBlobStorageBackend) -> None:
    ...
    async def test_download_not_found(self, backend: AzureBlobStorageBackend) -> None:
        ...
        with patch.object(backend, "_get_blob_client", return_value=mock_blob), \
             patch("azure.core.exceptions.ResourceNotFoundError", MockResourceNotFoundError):
            with pytest.raises(FileNotFoundError):
                await backend.download("missing.txt")
    ...
    @pytest.mark.asyncio
    async def test_get_presigned_url_get(self, backend: AzureBlobStorageBackend) -> None:
        """Test Azure presigned URL for GET."""
        pytest.importorskip("azure.storage.blob")
        
        with patch("azure.storage.blob.BlobSasPermissions") as MockPerms, \
    ...
    @pytest.mark.asyncio
    async def test_get_presigned_url_put(self, backend: AzureBlobStorageBackend) -> None:
...
class TestCreateStorageBackend:
    ...
    def test_loads_config_from_env_if_none(self) -> None:
        ...
        with patch.dict(os.environ, {"STORAGE_BACKEND": "local", "STORAGE_LOCAL_PATH": "/tmp/test"}), \
