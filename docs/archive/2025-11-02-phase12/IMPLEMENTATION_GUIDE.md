...
### Data Pipeline Enhancements (Nov 2025)
- All ingest scripts now surface a consistent reminder to run `post_ingest_checklist.py`, which wraps dataset health, event loader regression, and sampling-weight regeneration.
- Use the checklist immediately after Cambridge completes so the merged manifests feed directly into `derive_sampling_weights` without manual shell juggling.
- While Cambridge is running, review `training/configs/sampling_profiles.json` for any clamps/boosts that should change once the new counts land; jot findings in `training/reports/health/` for traceability.
- Capture ingest throughput metrics (disk %, avg MB/s) in `SESSION_SUMMARY` notes so we can tune `--workers` on future runs.
