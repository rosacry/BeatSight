...
export function PhoneVerificationSettings() {
    ...
    const handleRemovePhone = async () => {
        if (!accessToken) return
        if (!confirm('Are you sure you want to remove your phone number? This will affect your ability to vote on beatmap accuracy.')) {
            return
        }
            ...
            {!isVerified && (
                <div className="mb-4 p-3 rounded-lg bg-primary-500/10 border border-primary-500/20">
                    <p className="text-sm text-primary-300">
                        <strong>Why verify your phone?</strong> Users with both email and phone verified
                        receive a <span className="text-primary-400 font-semibold">200 karma bonus</span> and
                        can participate in beatmap accuracy voting.
                    </p>
                </div>
            )}
            ...
            {isVerified && user?.email_verified && (
                <div className="mt-4 p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                    <p className="text-sm text-green-400">
                        ✓ You have both email and phone verified! You can now participate in beatmap accuracy voting.
                    </p>
                </div>
            )}
            ...
            {isVerified && !user?.email_verified && (
                <div className="mt-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                    <p className="text-sm text-amber-400">
                        Verify your email address to receive the 200 karma bonus and unlock beatmap accuracy voting.
                    </p>
                </div>
            )}
        </div>
